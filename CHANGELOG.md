...
- `jj restore <paths>` now snapshots only the selected paths. Invalid filesets
  are rejected before snapshotting, and unrelated files no longer affect the
  operation.
...
- New _experimental_ `jj git fetch --tag` flag to fetch tags in the same way as
