            ...
            match format {
                ...
                DiffFormat::StatSummary(options) => {
                    ...
                    let stats =
                        DiffStats::calculate(store, tree_diff, options, self.conflict_marker_style)
                            .block_on()?;
                    show_diff_stat_summary(formatter, &stats, path_converter)?;
                }
                DiffFormat::Types => {
                    let tree_diff = diff_stream();
...
pub fn show_diff_stat_summary(
    formatter: &mut dyn Formatter,
    stats: &DiffStats,
    _path_converter: &RepoPathUiConverter,
) -> io::Result<()> {
    let total_added = stats.count_total_added();
    let total_removed = stats.count_total_removed();
    let total_files = stats.entries().len();
    ...
    writeln!(
        formatter.labeled("stat-summary"),
        "{total_files} files changed, {total_added} insertions(+), {total_removed} deletions(-)",
    )?;
    Ok(())
}
