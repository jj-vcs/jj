...
fn show_unified_diff_hunks(
    ...
    for hunk in unified_diff_hunks(contents, options.context, options.line_diff.compare_mode) {
        ...
        if let Some(source_symbol) = source_symbol {
            ...
            // Treat a consecutive run of non-ASCII bytes as one unit. If the
            // limit splits a run, omit the whole run.
            if end > 0
                && source_symbol.get(end).is_some_and(|byte| *byte >= 0x80)
                && source_symbol[end - 1] >= 0x80
            {
