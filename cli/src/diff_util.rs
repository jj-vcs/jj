...
pub fn show_structured_conflicts_diff(
    ...
    // TODO: conflict_marker_style
) -> Result<(), DiffRenderError> {
    let conflict_marker_style = ConflictMarkerStyle::Diff;
    async {
        while let Some(TreeDiffEntry { path, values: diff }) = tree_diff.next().await {
            ...
            if left_value.is_absent() {
                ...
                let right_content = diff_content(&path, right_value, conflict_marker_style)?;
                ...
                let left_content = diff_content(&path, left_value, conflict_marker_style)?;
                let right_content = diff_content(&path, right_value, conflict_marker_style)?;
