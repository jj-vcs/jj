...
pub fn is_alias_enabled(config: &StackedConfig, alias_name: &str) -> bool {
    // Get `aliases.<name>.enabled`. If it exists, and it's a bool, and the bool
    // value is `false`, then the alias is disabled. In all other cases, even
    // other invalid types, it should be considered enabled.
    !config
        .get_value(["aliases", alias_name, "enabled"])
        .is_ok_and(|value| value.as_bool().is_some_and(|enabled| !enabled))
}
