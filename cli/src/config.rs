...
impl UnresolvedConfigEnv {
    fn resolve(self) -> Vec<ConfigPath> {
        ...
        use ConfigPath::*;
        if let Some(Existing(path)) = home_config_path {
            paths.push(Existing(path));
        } else if let (Some(New(path)), None) = (home_config_path, &platform_config_path) {
            paths.push(New(path));
        }
...
mod tests {
    ...
    fn test_config_path_jj_config_existing() -> anyhow::Result<()> {
        TestCase {
            files: ["custom1.toml"],
            env: UnresolvedConfigEnv {
                jj_config: Some("custom1.toml".into()),
                ..Default::default()
            },
            wants: [Want::Existing("custom1.toml")],
        }
        .run()
    }
    ...
    fn test_config_path_jj_config_existing_multiple() -> anyhow::Result<()> {
        TestCase {
            ...
            env: UnresolvedConfigEnv {
                jj_config: join_paths(["custom1.toml", "custom2.toml"])
                    .unwrap()
                    .into_string()
                    .ok(),
                ..Default::default()
