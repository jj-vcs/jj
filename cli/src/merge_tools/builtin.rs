...
mod tests {
    ...
    fn test_edit_diff_builtin_empty_file_mode_change() {
        ...
        let left_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder
                .entry(empty_file_path)
                .empty_file()
                .executable(false);
        });
        let right_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder.entry(empty_file_path).empty_file().executable(true);
        });
    ...
    fn test_edit_diff_builtin_binary_file_mode_change() {
        ...
        let left_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder
                .entry(binary_file_path)
                .binary_file(vec![0xff, 0x00])
                .executable(false);
        });
        let right_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder
    ...
    fn test_edit_diff_builtin_change_binary_file_with_unselected_file_mode_change() {
        ...
        let expected_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder
                .entry(binary_file_path)
                .binary_file(vec![0xff, 0x01])
                .executable(false);
    ...
    fn test_edit_diff_builtin_delete_binary_file_with_unselected_file_mode_change() {
        ...
        let expected_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder.entry(binary_file_path).empty_file();
        });
        let actual_tree_id = apply_diff(store, &left_tree, &right_tree, &changed_files, &files);
        let actual_tree = store.get_root_tree(&actual_tree_id).unwrap();
        assert_eq!(
    ...
    fn test_edit_diff_builtin_replace_directory_with_file() {
        ...
        let left_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder.entry(&file_in_folder_path).empty_file();
        });
        let right_tree = testutils::create_tree(&test_repo.repo, |builder| {
            builder.entry(folder_path).empty_file();
        });
