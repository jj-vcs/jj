...
pub(crate) fn check_out_trees(
    ...
    let check_out = |name: &str, tree, files, read_only| -> Result<TreeState, DiffCheckoutError> {
        ...
        let mut tree_state = TreeState::init(store.clone(), wc_dir, state_dir, options.exec_config)?;
        tree_state.set_sparse_patterns(files, options)?;
...
impl DiffEditWorkingCopies {
    ...
    pub fn check_out(
        ...
        let diff_wc = check_out_trees(store, left_tree, right_tree, matcher, diff_type, options)?;
