...
/// parent revision.
///
/// Moves changes from `--from` revisions to the `--into` revision.
/// If `--into` is not specified, it defaults to `roots(from)-`.
...
pub(crate) struct SquashArgs {
    ...
    /// Revision to squash into (default: `roots(from)-`)
...
pub const LOCAL_VARIABLE_FROM: &str = "from";
pub const INTO_DEFAULT: &str = "roots(from)-";
...
pub(crate) async fn cmd_squash(
    ...
    if insert_destination_commit {
        ...
        let into_str = args
            ...
            .as_ref()
            .map(|arg| arg.as_ref())
            .unwrap_or(INTO_DEFAULT);

        let source_id_hexes = sources.iter().map(|commit| commit.id().hex()).collect_vec();
        let sources_exp_node = ExpressionNode {
            kind: ExpressionKind::UnionAll(
                source_id_hexes
                    .iter()
                    .map(|commit_id_hex| ExpressionNode {
                        kind: ExpressionKind::String(commit_id_hex.clone()),
                        span: Span::new(commit_id_hex, 0, commit_id_hex.len())
                            .expect("programmatic span shouldn't fail"),
                    })
                    .collect_vec(),
            ),
            // This is ugly, but I cannot find a reasonable alternative without constructing the
            // full "a|b|c"-like String to represent the span.
            span: Span::new(LOCAL_VARIABLE_FROM, 0, LOCAL_VARIABLE_FROM.len())
                .expect("programmatic span shouldn't fail"),
        };
