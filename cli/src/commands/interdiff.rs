...
/// the two revisions `A` and `B` have different parents `P` and `Q`:
/// `jj diff --from A --to B` will include the changes between their parents
/// (`P` to `Q`), while `jj interdiff --from A --to B` will not.
///
/// Technically, this works by rebasing `A` onto `B`'s parent(s) `Q`
/// and comparing the rebased revision `A'` to `B`:
///
/// ```text
/// A(from) B(to)      A'(A rebased onto Q)      diff(Q -> B)
/// |       |      =>  |                     =>  :
/// P       Q          | B(to)                   : (compare diffs)
///                    |/                        :
///                    Q                         diff(Q -> A')
/// ```
///
