...
///
/// The target `--to` defaults to `revsets.advance-default-to`
/// (which defaults to `@`).
///
/// The bookmarks to advance are determined by `revsets.advance-default-from`
...
/// `jj bookmark move --from 'heads(::to & bookmarks(x))' --to x`.
...
pub fn cmd_bookmark_advance(
    ...
    let matched_bookmarks = {
        let mut bookmarks: Vec<_> = match &args.names {
            ...
            None => {
                // Get the default-from revset template, and provide `to`.
                let from_template = workspace_command
                    .settings()
                    .get_string("revsets.advance-default-from")?;
                ...
                let expression = revset::parse(&mut diags, &from_template, &context)?;
                print_parse_diagnostics(ui, "In revsets.advance-default-from", &diags)?;
