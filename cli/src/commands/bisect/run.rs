...
/// assumed to be "good". The program will then look for the first "good"
/// revision.
///
...
pub(crate) struct BisectRunArgs {
    ...
    /// that turned the working directory from good to bad. With `--find-good`,
    /// the program instead assumes that the working directory changed from
    /// bad to good.
    ///
