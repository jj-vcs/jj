...
pub struct GitInitArgs {
    ...
    /// Disable colocation for the workspace of the Jujutsu repo with the Git
    /// repo
    ///
    ...
    /// will work on the repo. The `git.colocate` setting is ignored in this
    /// case.
    ///
    /// If the specified `--git-repo` path is outside of the given
    /// `DESTINATION`, then the Jujutsu repo will store a reference to the
    /// external Git repository (not a colocated workspace). It is an error to
    /// provide `--colocate` in this case. To convert an existing Git checkout
    /// in place, prefer running `jj git init` from that directory instead.
    ///
    /// [colocated workspace]:
    ///     https://docs.jj-vcs.dev/latest/git-compatibility/#colocated-jujutsugit-workspaces
    #[arg(
