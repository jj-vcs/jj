...
async fn cmd_git_colocation_enable(
    ...
    if is_child_workspace {
        if git_backend.git_repo().workdir().is_none() {
            return Err(user_error("Cannot colocate: bare Git repository"));
        }
...
async fn cmd_git_colocation_disable(
    ...
    if is_child_workspace {
        let git_backend = git::get_git_backend(workspace_command.repo().store())?;
        if git_backend.git_repo().workdir().is_none() {
            return Err(user_error("Cannot disable colocation: bare Git repository"));
        }
        let subprocess_options = GitSubprocessOptions::from_settings(workspace_command.settings())?;
        let workspace_root = workspace_command.workspace_root().to_owned();
