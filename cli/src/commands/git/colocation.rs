...
fn cmd_git_colocation_enable(
    ...
    if let Err(e) = std::fs::rename(&git_store_path, &dot_git_path) {
    ...
    // Get the working copy commit's parent ID before starting the transaction to
    // avoid borrowing issues
    let wc_commit = workspace_command
        .repo()
        .store()
        .get_commit(workspace_command.get_wc_commit_id().unwrap())?;
    let parent_id = wc_commit.parent_ids()[0].clone();
    set_git_head_to_commit_id(workspace_command, &parent_id)?;
...
fn cmd_git_colocation_disable(
    ...
    // Finally, remove the git HEAD reference
    let mut tx = workspace_command.start_transaction();
    tx.repo_mut().set_git_head_target(RefTarget::absent());
    if tx.repo().has_changes() {
        tx.finish(ui, "git colocation disable: remove Git head reference")?;
    } else {
        drop(tx);
    }
