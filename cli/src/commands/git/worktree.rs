...
use crate::commands::git::write_jj_gitignore;
...
fn list_git_linked_worktrees(repo: &ReadonlyRepo) -> Result<Vec<GitLinkedWorktree>, CommandError> {
    ...
    for proxy in proxies {
        ...
        let Ok(worktree_root) = dunce::canonicalize(base) else {
            continue;
        };
...
async fn resolve_repo_context<'a>(
    ...
    if let Ok(workspace) = command.load_workspace() {
        ...
        let main_git_repo = git_backend.git_repo();
        let main_workspace_root = main_git_repo
            .workdir()
...
async fn cmd_git_worktree_adopt(
    ...
    if args.names.is_empty() && !args.all {
        return cmd_git_worktree_adopt_cwd(ui, &ctx, command.cwd()).await;
    }
    ...
    for wt in &to_adopt {
        ...
        write_jj_gitignore(&wt.worktree_root)?;
        writeln!(
            ui.status(),
...
async fn cmd_git_worktree_adopt_cwd(
    ...
    .await?;
    // The adopted directory is a Git worktree, so keep Git from tracking the
    // workspace's own .jj directory.
    write_jj_gitignore(&wt.worktree_root)?;
    writeln!(
        ui.status(),
        r#"Created jj workspace for Git worktree at "{}"."#,
