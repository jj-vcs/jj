...
pub struct SignArgs {
    ...
    ///
    /// Note that revisions, which are already signed by you, will be resigned.
    ///
...
pub fn cmd_sign(ui: &mut Ui, command: &CommandHelper, args: &SignArgs) -> Result<(), CommandError> {
    ...
    if let Some(mut formatter) = ui.status_formatter() {
        if !signed_commits.is_empty() {
            ...
            writeln!(formatter, "Signed {} commit(s):", signed_commits.len())?;
    ...
    let num_not_authored_by_me = to_sign
        .iter()
