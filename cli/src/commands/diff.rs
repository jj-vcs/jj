...
pub(crate) struct DiffArgs {
    ...
    /// A..D, then `jj diff -r B::D` equals `jj diff --from A --to D`.
    ///
    ...
    /// root. For the purpose of this command, we call such revsets "linear
...
pub(crate) fn cmd_diff(
    ...
    if show_nonlinear_revset_warning {
        ...
        writeln!(
            ui.warning_default(),
            "Showing combined diff of multiple revisions that do not form a linear sequence"
        )?;
    }
