...
/// and return the updated file content on standard output. This is requirement
/// enables the possibility to fix any commit. The working copy does not need
/// to change, we do not rely on its content.
///
/// A tool's output will not be used unless it exits with a successful exit
/// code. Output on standard error will be passed through to the terminal.
///
...
///
/// The following configuration defines how two code formatters
...
/// ### Execution example
///
/// Let's take the configuration example above and see what would happen to the
/// following history.
///
/// ```text
/// C (mutable)
...
/// ```
///
/// By default, `jj fix` will modifies commits that matches the revset
/// `reachable(@, mutable())` (see `jj help -k revsets`) which corresponds to
/// the commits `A`, `B` and `C` here.
///
/// The following operations will then happen:
///
/// - For commit `A`, content from this commit for files `src/bar.cc` and
///   `src/bar.h` will each be provided to `clang-format` and the result output
///   will be used to recreate commit `A` which we will call `A'`. All other
///   files are untouched.
/// - For commit `B`, same thing happen for files `src/bar.cc` and `src/bar.h`
///   Their content from commit `B` will go through `clang-format`. The file
///   `README.md` as any other files, are untouched as no pattern matches it. We
///   obtain commit `B'`.
/// - For commit `C`, `src/bar.cc` and `src/bar.h` goes through `clang-format`
...
/// ```
///
/// Your commits are now all correctly formatted according to your
/// configuration.
