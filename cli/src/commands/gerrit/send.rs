...
pub fn cmd_send(ui: &mut Ui, command: &CommandHelper, send: &SendArgs) -> Result<(), CommandError> {
    ...
    for commit_id in to_send.iter().map(|c| c.id()).rev() {
        ...
        // JJ change-IDs have a length of 32 in hex form.
        // Gerrit requires a 40-character hex representation.
        let change_id = original_commit.change_id().hex();
        let zfill = "0".repeat(40 - change_id.len());
        let gerrit_change_id = format!("I{zfill}{change_id}");
        ...
        let new_commit = tx
            ...
            // Otherwise, `jj gerrit send @ && jj gerrit send @` will upload
