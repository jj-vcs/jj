...
pub struct UploadArgs {
    ...
    /// Add these emails as a reviewer.
    #[arg(long)]
    reviewer: Vec<String>,

    /// CC these emails on the change.
    #[arg(long)]
    cc: Vec<String>,

    /// Add the following label configured by gerrit.
    /// Defaults to +1 if no value is set.
    /// Eg. --label=Auto-Submit will set the Auto-Submit label (if it exists
    /// on your gerrit host)
    #[arg(long, short = 'l')]
    label: Vec<String>,
    ...
    /// Who to email notifications to. If not provided, defaults to all.
    #[arg(long, value_enum)]
    ...
    /// Send the following custom keyed values to gerrit.
...
/// Which emails receive an email notification about an update to a change.
...
fn push_options(args: &UploadArgs) -> Result<Vec<String>, CommandError> {
    ...
    .map(|item| match item {
        Some((k, v)) => vec!["-o".to_string(), format!("{k}={v}")],
        None => vec![],
    })
    .chain(
        [
