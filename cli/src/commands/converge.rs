...
/// a given change with a single commit. This command applies heuristics to try
/// to automatically come up with a good solution, and falls back to prompting
/// the user if the heuristics are inconclusive. Use `--no-interactive` to run
/// in non-interactive mode.
///
/// Returns successfully if there are no divergent commits in the specified
/// revisions.
...
pub(crate) struct ConvergeArgs {
    ...
    /// setting. jj converge evaluates these revsets and then groups the commits
    /// by change id. Change ids with more than one commit are considered
    /// divergent. If multiple divergent changes exist, prompts the user to
    /// choose one. Note that commits not matching these revisions are ignored.
