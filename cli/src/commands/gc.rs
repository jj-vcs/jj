...
use jj_lib::gc::find_discard_op;
use jj_lib::gc::write_last_gc_time;
use jj_lib::op_walk;
...
pub(crate) async fn run_gc(
    ...
    let settings = GcSettings::from_user_settings(repo.settings())?;
    ...
    let discard_op =
        find_discard_op(&head_ops, settings.operation_min_count, expiry_cutoff).await?;
    ...
    reindex_at_operation(repo.loader(), repo.operation()).await?;

    write_last_gc_time(repo_path).map_err(|err| user_error(err.to_string()))?;
