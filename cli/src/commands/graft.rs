...
pub(crate) async fn cmd_graft(
    ...
    match subcommand {
        GraftCommand::Tree(args) => cmd_graft_tree(ui, command, args),
    }
}
...
fn cmd_graft_tree(
    ...
    let destination_commit = workspace_command.resolve_single_rev(ui, &args.destination)?;
    ...
    if grafted_count == 0 {
        writeln!(ui.status(), "No revisions to graft.")?;
        tx.finish(ui, "graft (no changes)")?;
        return Ok(());
    }

    tx.finish(ui, format!("graft {grafted_count} commit(s)"))?;
    Ok(())
}
