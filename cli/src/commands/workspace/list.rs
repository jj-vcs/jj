...
pub struct WorkspaceListArgs {
    ...
    /// more information.
    ///
    /// Can be overridden by the `templates.workspace_list` setting. The
    /// default is exposed as the `builtin_workspace_list` alias, so user
    /// templates can wrap it instead of replacing it.
    ///
    /// [`WorkspaceRef` type]:
