...
pub struct WorkspaceAddArgs {
    ...
    /// colocated with Git.
    ///
    /// **This is the default** when the [git.colocate config] is `true`.
    ///
    /// [git.colocate config]:
    ...
    ///
    /// **This is the default** when the [git.colocate config] is `false` (the
    /// default).
    ///
    /// [git.colocate config]:
    ///     https://docs.jj-vcs.dev/latest/config/#default-colocation
...
pub fn cmd_workspace_add(
    ...
    // Validate and set up --colocate if requested (via flag or config)
    // The guard will clean up the worktree if we return early due to an error
    #[cfg(feature = "git")]
    let worktree_guard = {
        // Determine if colocation is requested (explicit flag takes precedence over
        // config)
        let colocate_requested = if command.settings().get_bool("git.colocate")? {
            !args.no_colocate
        } else {
            args.colocate
        };
        let explicit_colocate_flag = args.colocate;

        if colocate_requested {
            let is_colocated =
                is_colocated_git_workspace(None, old_workspace_command.workspace(), repo.as_ref());
            if !is_colocated {
                if explicit_colocate_flag {
                    // User explicitly asked for --colocate on non-colocated repo
                    return Err(user_error(
                        "Cannot use --colocate: repository is not colocated with Git",
                    ));
                } else {
                    // Config default - warn and proceed without colocation
                    writeln!(
                        ui.warning_default(),
                        "git.colocate config is true, but the parent repository is not colocated \
                         with Git. The new workspace will not be colocated."
                    )?;
                }
            }
            ...
            if is_colocated {
                ...
                // Colocation was requested via config but repo isn't colocated, skip worktree
