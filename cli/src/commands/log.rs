...
enum DisplayNode {
    ...
    Present(CommitId),
    /// Prints the "(elided revisions)" between .0 (child) and .1 (eventual
    /// present parent)
    IndirectPath(CommitId, CommitId),
    /// Prints the representation of all elided parents of .0
        ...
        for edge in edges {
            if edge.is_direct() {
                ...
                if !use_elided_nodes {
                    ...
                    let synthetic_node =
                        DisplayNode::IndirectPath(node.clone(), edge.target.clone());
                    let real_target = DisplayNode::Present(edge.target);
                    pending
                        .push_back((synthetic_node.clone(), vec![GraphEdge::direct(real_target)]));
                    new_edges.push(GraphEdge::direct(synthetic_node));
                }
            } else if use_elided_nodes && !has_missing_parents {
