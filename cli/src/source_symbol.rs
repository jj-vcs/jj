...
impl<'a> SourceSymbolScanner<'a> {
    ...
    /// Finds the closest symbol before the range, including its first line.
    ///
    ...
    pub(crate) fn find(&self, line_range: &Range<usize>) -> Option<&'a [u8]> {
        let preceding_end = self.symbols.partition_point(|(line, _)| {
        ...
        if let Some((_, symbol)) = preceding_end
            .checked_sub(1)
