...
#[test]
fn test_git_worktree_adopt_uses_current_directory_name() {
    let test_env = TestEnvironment::default();
    test_env
        .run_jj_in(".", ["git", "init", "--colocate", "main"])
        .success();
    let main_dir = test_env.work_dir("main");
    let original_dir = test_env.work_dir("original");
    let renamed_dir = test_env.work_dir("renamed");

    main_dir.write_file("file", "contents");
    main_dir.run_jj(["commit", "-m", "initial"]).success();
    git::add_worktree(main_dir.root(), original_dir.root(), Some("HEAD"));
    let output = std::process::Command::new("git")
        .args(["worktree", "move"])
        .arg(original_dir.root())
        .arg(renamed_dir.root())
        .current_dir(main_dir.root())
        .output()
        .expect("git worktree move failed to spawn");
    assert!(
        output.status.success(),
        "git worktree move failed: {output:?}"
    );

    main_dir
        .run_jj(["git", "worktree", "adopt", "original"])
        .success();
    insta::assert_snapshot!(main_dir.run_jj(["workspace", "list"]).normalize_backslash(), @r#"
    default: . rlvkpnrz 504e3d8c (empty) (no description set)
    renamed: ../renamed pmmvwywv 058f604d (empty) (no description set)
    [EOF]
    "#);
}

#[test]
fn test_git_worktree_adopt_with_workspace_name() {
    let test_env = TestEnvironment::default();
    test_env
        .run_jj_in(".", ["git", "init", "--colocate", "main"])
        .success();
    ...
    main_dir
        .run_jj(["git", "worktree", "adopt", "--name", "custom", "linked"])
        .success();
    insta::assert_snapshot!(main_dir.run_jj(["workspace", "list"]).normalize_backslash(), @r#"
    custom: ../linked pmmvwywv 058f604d (empty) (no description set)
    default: . rlvkpnrz 504e3d8c (empty) (no description set)
    [EOF]
    "#);
}
...
#[test]
fn test_git_worktree_adopt_avoids_workspace_name_collision() {
    let test_env = TestEnvironment::default();
    ...
    main_dir.run_jj(["commit", "-m", "initial"]).success();
    main_dir
        .run_jj(["workspace", "add", "--name", "linked", "../other"])
        .success();
    git::add_worktree(main_dir.root(), linked_dir.root(), Some("HEAD"));
    ...
    insta::assert_snapshot!(main_dir.run_jj(["workspace", "list"]).normalize_backslash(), @r#"
    default: . rlvkpnrz 504e3d8c (empty) (no description set)
    linked: ../other pmmvwywv 058f604d (empty) (no description set)
    linked-2: ../linked rzvqmyuk bcc858e1 (empty) (no description set)
    [EOF]
    "#);
}
...
fn test_git_worktree_adopt_already_exists() {
    ...
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Error: Git worktree 'linked' is already adopted as workspace 'linked'
    [EOF]
    [exit status: 1]
    ");
}
...
#[test]
fn test_git_worktree_sync_adopts_all() {
    let test_env = TestEnvironment::default();
    test_env
        .run_jj_in(".", ["git", "init", "--colocate", "main"])
        .success();
    ...
    let output = main_dir.run_jj(["git", "worktree", "sync"]);
    insta::assert_snapshot!(output.normalize_backslash(), @r#"
    ------- stderr -------
    Created workspace in "../linked"
    [EOF]
    ...
    insta::assert_snapshot!(main_dir.run_jj(["git", "worktree", "sync"]), @"");
}
...
fn test_git_worktree_custom_workspace_name_not_forgotten() {
    ...
    let output = main_dir.run_jj(["git", "worktree", "adopt", "bar"]);
    insta::assert_snapshot!(output, @r#"
    ------- stderr -------
    Error: Git worktree 'bar' is already adopted as workspace 'foo'
    [EOF]
    [exit status: 1]
    "#);
...
#[test]
fn test_git_worktree_sync_repairs_custom_workspace_path() {
    let test_env = TestEnvironment::default();
    test_env.add_config("git.colocate = true");
    test_env
        .run_jj_in(".", ["git", "init", "--colocate", "main"])
        .success();
    let main_dir = test_env.work_dir("main");
    let original_dir = test_env.work_dir("bar");
    let moved_dir = test_env.work_dir("nested/baz");
    ...
    insta::assert_snapshot!(main_dir.run_jj(["workspace", "list"]).normalize_backslash(), @r#"
    default: . rlvkpnrz 504e3d8c (empty) (no description set)
    foo: ../nested/baz pmmvwywv 058f604d (empty) (no description set)
    [EOF]
    "#);
