...
fn test_workspaces_conflicting_edits() {
    ...
    Inspect the saved commit with `jj show 5f9e41510c33`.
    [EOF]
    ");
...
fn test_workspaces_updated_by_other_with_changes_in_working_copy_automatic() {
    ...
    Inspect the saved commit with `jj show 8e2b91d60d5d`.
...
fn test_workspaces_update_stale_recovers_changes() {
    ...
    Inspect the saved commit with `jj show 9ae6e8703803`.
    [EOF]
    ");
    ...
    // The suggested command shows the saved edit without changing the working copy.
    let output = secondary_dir.run_jj(["show", "9ae6e8703803"]).success();
    assert!(output.stdout.raw().contains("precious changes"));
    insta::assert_snapshot!(secondary_dir.read_file("file"), @"changed in main");

    // Restoring from the hinted commit can bring the edit back in this case.
    secondary_dir
