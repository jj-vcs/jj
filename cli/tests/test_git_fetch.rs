...
fn test_git_fetch_no_git_executable() {
    ...
    let stderr = test_env.jj_cmd_failure(&repo_path, &["git", "fetch"]);
    insta::assert_snapshot!(stderr, @"Error: Could not find 'git' executable in the OS path");
}

#[test]
fn test_git_fetch_no_git_executable_with_path() {
    let mut test_env = TestEnvironment::default();
    let invalid_git_path = if cfg!(windows) {
        "C:\\this\\is\\an\\invalid\\path"
    } else {
        "/this/is/an/invalid/path"
    };
    test_env.add_config("git.subprocess = true");
    test_env.add_config(format!("git.executable_path = '{invalid_git_path}'"));
    test_env.jj_cmd_ok(test_env.env_root(), &["git", "init", "repo"]);
    let repo_path = test_env.env_root().join("repo");
    add_git_remote(&test_env, &repo_path, "origin");
    ...
    let stderr = test_env.jj_cmd_failure(&repo_path, &["git", "fetch"]);
    if cfg!(windows) {
        insta::assert_snapshot!(stderr, @r"Error: Could not execute git executable at path 'C:\this\is\an\invalid\path'");
    } else {
        insta::assert_snapshot!(stderr, @"Error: Could not execute git executable at path '/this/is/an/invalid/path'");
    }
}
