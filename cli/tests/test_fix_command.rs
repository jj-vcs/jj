...
fn test_fix_with_line_ranges_and_include_unchanged_files() {
    ...
    let output = work_dir
        .run_jj(["fix", "--include-unchanged-files", "-s", "c2"])
        .success();
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Fixed 1 commits of 1 checked.
    Working copy  (@) now at: kkmpptxz a521be01 c2 | (no description set)
    ...
    // Check that the formatter was applied to the second commit.
    // changed.txt, newfile.txt should have only changed lines affected.
    ...
    // Unchanged file (unchanged.txt) should not be affected.
