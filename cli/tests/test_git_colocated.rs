...
fn test_workspace_forget_removes_git_worktree() {
    ...
    // 4. Forget the workspace (use --force since jj-checked-out files are
    // untracked from git's perspective)
    primary_work_dir
        .run_jj(["workspace", "forget", "--force", "second"])
        .success();
...
fn test_workspace_forget_with_custom_name_removes_git_worktree() {
    ...
    // Use --force since jj-checked-out files are untracked from git's perspective
    primary_work_dir
        .run_jj(["workspace", "forget", "--force", "myworkspace"])
        .success();
