...
fn test_track_ignored_with_flag() {
    ...
    // Snapshot to track the non-ignored file
    work_dir.run_jj(["st"]).success();
    let output = work_dir.run_jj(["file", "list"]);
    insta::assert_snapshot!(output, @r"
    .gitignore
    ...
    // File is still tracked after modifying it
    work_dir.write_file("file2.ignored", "modified content");
    let output = work_dir.run_jj(["status"]);
    assert!(output.stdout.raw().contains("file2.ignored"));
}
...
fn test_track_large_file_with_flag() {
    ...
    // Snapshot to track the small file
    work_dir.run_jj(["st"]).success();
    let output = work_dir.run_jj(["file", "list"]);
    insta::assert_snapshot!(output, @r"
    small.txt
