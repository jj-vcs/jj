...
fn test_conflict_marker_length_stored_in_working_copy() {
    ...
    Normal { <executable> has_crlf_in_store: false }           249 <timestamp> Some(MaterializedConflictData { conflict_marker_len: 11 }) "file"
    [EOF]
    "#);
    ...
    Normal { <executable> has_crlf_in_store: false }           289 <timestamp> Some(MaterializedConflictData { conflict_marker_len: 11 }) "file"
    [EOF]
    "#);
    ...
    Normal { <executable> has_crlf_in_store: false }           130 <timestamp> None "file"
    [EOF]
    "#);
}
