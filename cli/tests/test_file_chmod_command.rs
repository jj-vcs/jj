...
#[cfg(unix)]
#[test]
fn test_chmod_executable_bit_settings() {
    let test_env = TestEnvironment::default();
    test_env.run_jj_in(".", ["git", "init", "repo"]).success();
    let work_dir = test_env.work_dir("repo");
    ...
    Current tree: Merge(Resolved(TreeId("6d5f482d15035cdd7733b1b551d1fead28d22592")))
    Normal { exec_bit: ExecBit { in_repo: false, on_disk: false } }             5 <timestamp> None "file"
    [EOF]
    "#); // false false (1/4)
    ...
    Current tree: Merge(Resolved(TreeId("5201dbafb66dc1b28b029a262e1b206f6f93df1e")))
    Normal { exec_bit: ExecBit { in_repo: true, on_disk: true } }             5 <timestamp> None "file"
    [EOF]
    "#); // true true (2/4)
    ...
    Current tree: Merge(Resolved(TreeId("6d5f482d15035cdd7733b1b551d1fead28d22592")))
    Normal { exec_bit: ExecBit { in_repo: false, on_disk: true } }             5 <timestamp> None "file"
    [EOF]
    "#); // false true (3/4)
    ...
    Current tree: Merge(Resolved(TreeId("5201dbafb66dc1b28b029a262e1b206f6f93df1e")))
    Normal { exec_bit: ExecBit { in_repo: true, on_disk: false } }             5 <timestamp> None "file"
    [EOF]
    "#); // true false (4/4) Yay! We've observed all possible ExecBit states!
    ...
    work_dir.run_jj(["file", "chmod", "x", "file"]).success();
    assert_file_executable(path, true);
    let output = work_dir.run_jj(["new", "base"]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Working copy  (@) now at: uyznsvlq d26a1051 (empty) (no description set)
    ...
    let output = work_dir.run_jj(["debug", "local-working-copy"]);
    insta::assert_snapshot!(output.normalize_stdout_with(redact_timestamp), @r#"
    Current operation: OperationId("2bf9778a6e567b0301c034a1974ef87ba7285af80afb5678fd7c0ba518f43716678be37609a6302497a066e22822e6b8a28181fa1543b1d1d5ebcb1a74484fb5")
    Current tree: Merge(Resolved(TreeId("5201dbafb66dc1b28b029a262e1b206f6f93df1e")))
    Normal { exec_bit: ExecBit { in_repo: true, on_disk: true } }             5 <timestamp> None "file"
    [EOF]
    "#);
...
#[cfg(unix)]
#[test]
fn test_chmod_executable_bit_ignore() {
    let test_env = TestEnvironment::default();
    test_env.run_jj_in(".", ["git", "init", "repo"]).success();
    let work_dir = test_env.work_dir("repo");
    let path = &work_dir.root().join("file");
    ...
    // 1. Reverting to "in_repo: true, on_disk: false" works.
    ...
    // 2. Reverting to "in_repo: false, on_disk: true" works.
