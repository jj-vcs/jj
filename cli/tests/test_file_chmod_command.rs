...
fn test_chmod_regular_conflict() {
    ...
    insta::assert_snapshot!(output, @r"
    <<<<<<< Conflict 1 of 1
    %%%%%%% Changes from base (rlvkpnrz 1792382a) to side #1 (royxmykx 02247291)
    -base
    +x
    +++++++ Contents of side #2 (zsuskuln eb0ba805)
    n
...
fn test_chmod_file_dir_deletion_conflicts() {
    ...
    insta::assert_snapshot!(output, @r"
    <<<<<<< Conflict 1 of 1
    +++++++ Contents of side #1 (zsuskuln bc9cdea1)
    a
    %%%%%%% Changes from base (rlvkpnrz 1792382a) to side #2 (royxmykx d7d39332)
    -base
    >>>>>>> Conflict 1 of 1 ends
    [EOF]
    ");
