...
fn test_bisect_bad() {
    ...
    insta::assert_snapshot!(work_dir.run_jj(["bisect", "run", "--range=..", "--command=false", "--find-first=bad"]), @r"
    Now evaluating: royxmykx dffaa0d4 c | c
    The revision is bad.
...
fn test_bisect_run_find_first_good() {
    ...
    insta::assert_snapshot!(work_dir.run_jj(["bisect", "run", "--range=..", "--command=true", "--find-first=good"]), @r"
    Now evaluating: royxmykx dffaa0d4 c | c
