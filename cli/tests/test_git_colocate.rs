...
fn test_git_colocate_success() {
    ...
    let _output = test_env.run_jj_in(&workspace_root, ["git", "colocate"]);
...
#[test]
fn test_git_colocate_already_colocated() {
    let test_env = TestEnvironment::default();
    ...
    let output = test_env.run_jj_in(&workspace_root, ["git", "colocate"]);
    assert!(
        output
            .stderr
            .raw()
...
#[test]
fn test_git_colocate_with_existing_git_dir() {
    let test_env = TestEnvironment::default();
...
#[test]
fn test_git_colocate_undo_success() {
    let test_env = TestEnvironment::default();

    // Initialize and colocate a repo first
    let _ = test_env.run_jj_in(test_env.env_root(), ["git", "init", "repo"]);
    let workspace_root = test_env.env_root().join("repo");
    let _ = test_env.run_jj_in(&workspace_root, ["git", "colocate"]);
    ...
    // Run undo
    let _ = test_env.run_jj_in(&workspace_root, ["git", "colocate", "--undo"]);
...
#[test]
fn test_git_colocate_undo_not_colocated() {
    let test_env = TestEnvironment::default();

    // Initialize a regular jj repo (not colocated)
    let _ = test_env.run_jj_in(test_env.env_root(), ["git", "init", "repo"]);
    let workspace_root = test_env.env_root().join("repo");

    // Try to undo colocate when not colocated - should fail
    let output = test_env.run_jj_in(&workspace_root, ["git", "colocate", "--undo"]);
    assert!(
        output
            .stderr
            .raw()
            .contains("Repository is not co-located with Git")
    );
}
