...
fn test_git_colocate_disable_success() {
    ...
    let workspace_root = test_env.env_root().join("repo");
    let _ = work_dir.run_jj(["git", "colocate", "--enable"]);
    ...
    // Disable colocation
    let _ = work_dir.run_jj(["git", "colocate", "--disable"]);
...
fn test_git_colocate_status_non_colocated() {
    ...
    ");
    insta::assert_snapshot!(output.stderr,@"
        Hint: To enable colocation, run: `jj git colocate --enable`
        [EOF]");
}
...
fn test_git_colocate_status_colocated() {
    ...
    ");
    insta::assert_snapshot!(output.stderr,@"
        Hint: To disable colocation, run: `jj git colocate --disable`
        [EOF]");
}
...
fn test_git_colocate_both_flags_error() {
    ...
    assert!(
        output
            .stderr
            .raw()
            .contains("the argument '--enable' cannot be used with '--disable'")
    );
}
