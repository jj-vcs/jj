...
fn test_config() {
    ...
    let output = test_env.complete_fish(["config", "get", "w"]);
    insta::assert_snapshot!(output, @r"
    working-copy.fsmonitor	Whether to use an external filesystem monitor, useful for large repos
    working-copy.watchman.register-snapshot-trigger	Whether to use triggers to monitor for changes in the background.
    [EOF]
    ");

    let output = test_env.complete_fish(["config", "list", "w"]);
    insta::assert_snapshot!(output, @r"
    working-copy	Working copy settings
    working-copy.fsmonitor	Whether to use an external filesystem monitor, useful for large repos
    working-copy.watchman
    working-copy.watchman.register-snapshot-trigger	Whether to use triggers to monitor for changes in the background.
    [EOF]
    ");

    let output = test_env.complete_fish(["log", "--config", "w"]);
    insta::assert_snapshot!(output, @r"
    working-copy.fsmonitor=	Whether to use an external filesystem monitor, useful for large repos
    working-copy.watchman.register-snapshot-trigger=	Whether to use triggers to monitor for changes in the background.
    [EOF]
