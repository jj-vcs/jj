...
fn test_bookmark_advance_default() {
    ...
    // A -> zsuskuln.
    let output = work_dir.run_jj(["bookmark", "advance", "A", "-t", "zsuskuln"]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    ...
    let output = work_dir.run_jj([
        "--config=revsets.advance-default-to=none()",
        "bookmark",
        "advance",
    ]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Error: No suitable revision to advance to.
    Hint: `revsets.advance-default-to` controls the default target. You can also specify a specific target with `--to`.
    [EOF]
    [exit status: 1]
    ");
}
