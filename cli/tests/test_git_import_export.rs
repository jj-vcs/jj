...
fn get_git_repo_refs(git_repo: &gix::Repository) -> Vec<(String, CommitId)> {
    let mut refs: Vec<_> = git_repo
        ...
        .unwrap()
        .filter_ok(|git_ref| {
            let path = git_ref.name().to_path();
            let suffix = path.strip_prefix("refs").unwrap();
            let base = suffix
                .components()
                .next()
                .unwrap()
                ...
                .to_str()
                .unwrap();
            base == "tags" || base == "heads" || base == "remote"
        })
