...
fn init_git_repo_with_opts(git_repo_path: &Path, opts: InitOpts<'_>) -> gix::Repository {
    ...
    if let Some(path) = opts.workspace_root {
        ...
        let git_link_path = path.join(".git");
...
fn test_git_init_colocated_dirty_working_copy() {
    ...
    assert_eq!(
        ...
        git::GitStatus::new(
            "new-staged-file",
            git::GitStatusInfo::Index(git::IndexStatus::Addition),
        ),
        git::GitStatus::new(
            "some-file",
            git::GitStatusInfo::Index(git::IndexStatus::Modification),
        ),
        git::GitStatus::new(
            "unstaged-file",
            git::GitStatusInfo::Worktree(git::WorktreeStatus::Added),
        )
        }
    );
