...
fn test_attacker_has_signature() {
    ...
    Hint: Run `jj config edit --repo` to review and re-enable
    Warning: The workspace has moved from $TEST_ENV/repo/.jj/workspace to $TEST_ENV/evil/.jj/workspace. For security reasons, we have disabled the workspace config.
    Hint: Run `jj config edit --workspace` to review and re-enable
...
fn test_legacy_config_migration() {
    ...
    let new_config = work_dir.root().join(".jj/repo/config");
