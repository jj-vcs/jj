...
fn test_file_search() {
    ...
    [EOF]
    ");
    // -l on a binary match prints just the path (same shape as a text file).
    let output = work_dir.run_jj(["file", "search", "-l", "--pattern=hit", "binfile"]);
    insta::assert_snapshot!(output.normalize_backslash(), @"
    binfile
    [EOF]
    ");
...
fn test_file_search_conflicts_binary() {
    ...
    [EOF]
    ");
    // -l on binary conflict prints just the path.
    let output = work_dir.run_jj(["file", "search", "-l", "--pattern=hit"]);
    insta::assert_snapshot!(output.normalize_backslash(), @"
    binfile
    [EOF]
    ");
