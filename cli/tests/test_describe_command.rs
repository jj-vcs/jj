...
fn test_add_trailer() {
    ...
    let output = work_dir.run_jj([
        ...
        "Message from CLI",
        "--config",
        r#"templates.commit_trailers='"Signed-off-by: " ++ self.author().name() ++ " <" ++ self.author().email() ++ ">"'"#,
    ]);
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    ...
    // invalid generated trailers won't appear in the description
    let output = work_dir.run_jj([
        "describe",
        "--no-edit",
        "--config",
...
fn test_add_trailer_committer() {
    ...
    test_env.add_config(
        r#"[templates]
        commit_trailers = '''"Signed-off-by: " ++ self.committer().email()'''"#,
    );
