...
fn test_absorb_replace_single_line_hunk() {
    ...
    ------- stderr -------
    Absorbed changes into 1 revisions:
      qpvuntsm 2fb2dd65 (conflict) 1
    Rebased 1 descendant commits.
    Working copy  (@) now at: mzvwutvl 52437581 (empty) (no description set)
    Parent commit (@-)      : kkmpptxz e2cb56ca 2
    ...
    insta::assert_snapshot!(get_diffs(&work_dir, "mutable()"), @r"
    @  mzvwutvl 52437581 (empty) (no description set)
    ○  kkmpptxz e2cb56ca 2
    │  diff --git a/file1 b/file1
...
fn test_absorb_conflict() {
    ...
    Working copy  (@) now at: kkmpptxz 1d2f155d (conflict) (no description set)
    Parent commit (@-)      : qpvuntsm e35bcaff 1
    Added 0 files, modified 1 files, removed 0 files
    Warning: There are unresolved conflicts at these paths:
    file1    2-sided conflict
    New conflicts appeared in 1 commits:
      kkmpptxz 1d2f155d (conflict) (no description set)
    Hint: To resolve the conflicts, start by creating a commit on top of
    the conflicted commit:
      jj new kkmpptxz
    Then use `jj resolve`, or edit the conflict markers in the file directly.
...
fn test_absorb_deleted_file_with_multiple_hunks() {
    ...
    ------- stderr -------
    Absorbed changes into 2 revisions:
      kkmpptxz a9947061 (conflict) 2
      qpvuntsm cca207ed (conflict) 1
    Rebased 1 descendant commits.
    Working copy  (@) now at: zsuskuln 54d2eb00 (no description set)
    Parent commit (@-)      : kkmpptxz a9947061 (conflict) 2
    ...
    insta::assert_snapshot!(get_diffs(&work_dir, "mutable()"), @r"
    @  zsuskuln 54d2eb00 (no description set)
    │  diff --git a/file2 b/file2
    │  deleted file mode 100644
    │  index 0000000000..0000000000
    │  --- a/file2
