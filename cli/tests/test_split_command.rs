...
fn test_split_with_descendants() -> TestResult {
    ...
    │  Add file1
    │  -- operation 829427e5fee2 split commit 1d2499e72cefc8a2b87ebb47569140857b96189f
...
fn test_split_parallel_no_descendants() -> TestResult {
    ...
    │  TESTED=TODO
    │  -- operation 423d84e3ba48 split commit f5700f8ef89e290e4e90ae6adc0908707e0d8c85
...
fn test_split_with_bookmarks(bookmark_behavior: BookmarkBehavior) -> TestResult {
    ...
    match bookmark_behavior {
        BookmarkBehavior::LeaveBookmarkWithTarget => {
            ...
            insta::assert_snapshot!(output, @r#"
            ------- stderr -------
            Warning: Deprecated user-level config: split.legacy-bookmark-behavior is updated to split.identity-strategy = ["selected"]
        ...
        BookmarkBehavior::MoveBookmarkToChild => {
            ...
            insta::assert_snapshot!(output, @r#"
            ------- stderr -------
            Warning: Deprecated user-level config: split.legacy-bookmark-behavior is updated to split.identity-strategy = ["remaining"]
...
#[test]
fn test_split_identity_strategy() -> TestResult {
    let mut test_env = TestEnvironment::default();
    let edit_script = test_env.set_up_fake_editor();
    test_env.run_jj_in(".", ["git", "init", "main"]).success();
    let main_dir = test_env.work_dir("main");
    ...
    // 1. Test strategy ["selected"]: parent keeps Change ID and bookmark
    test_env.add_config(r#"split.identity-strategy = ["selected"]"#);
    std::fs::write(
        &edit_script,
        ["", "next invocation\n", "write\nsecond-commit"].join("\0"),
    )?;
    ...
    Remaining changes: zsuskuln 293f3d5b second-commit
    Working copy  (@) now at: zsuskuln 293f3d5b second-commit
    Parent commit (@-)      : qpvuntsm 54c7e261 *le-signet* | first-commit
    [EOF]
    ");
    insta::assert_snapshot!(get_log_output(&main_dir), @"
    @  zsuskulnrvyr false second-commit
    ○  qpvuntsmwlqt false *le-signet* first-commit
    ◆  zzzzzzzzzzzz true
    [EOF]
    ");

    // 2. Test strategy ["remaining"]: child keeps Change ID and bookmark
    main_dir.run_jj(["undo"]).success();
    test_env.add_config(r#"split.identity-strategy = ["remaining"]"#);
    std::fs::write(
        &edit_script,
        ["", "next invocation\n", "write\nsecond-commit"].join("\0"),
    )?;
    ...
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Selected changes : yqosqzyt 1bb7b7b4 first-commit
    Remaining changes: qpvuntsm 71bd393c *le-signet* | second-commit
    Working copy  (@) now at: qpvuntsm 71bd393c *le-signet* | second-commit
    Parent commit (@-)      : yqosqzyt 1bb7b7b4 first-commit
    [EOF]
    ");
    insta::assert_snapshot!(get_log_output(&main_dir), @"
    @  qpvuntsmwlqt false *le-signet* second-commit
    ○  yqosqzytrlsw false first-commit
    ◆  zzzzzzzzzzzz true
    [EOF]
    ");

    // 3. Test strategy ["selected"] with --parallel: selected keeps Change ID and
    //    bookmark
    main_dir.run_jj(["undo"]).success();
    test_env.add_config(r#"split.identity-strategy = ["selected"]"#);
    std::fs::write(
    ...
    let output = main_dir.run_jj(["split", "file2", "--parallel"]);
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Selected changes : qpvuntsm 192b9f8a *le-signet* | first-commit
    Remaining changes: znkkpsqq bc5f19e1 second-commit
    Working copy  (@) now at: znkkpsqq bc5f19e1 second-commit
    Parent commit (@-)      : zzzzzzzz 00000000 (empty) (no description set)
    Added 0 files, modified 0 files, removed 1 files
    [EOF]
    ");
    insta::assert_snapshot!(get_log_output(&main_dir), @"
    @  znkkpsqqskkl false second-commit
    │ ○  qpvuntsmwlqt false *le-signet* first-commit
    ├─╯
    ◆  zzzzzzzzzzzz true
    [EOF]
    ...
    // 4. Test strategy ["selected"] with move flags:
    // The extracted/selected commit retains the original Change ID and bookmark at
    // the new destination, while the origin commit receives a new Change ID.
    main_dir.run_jj(["undo"]).success();
    let output = main_dir.run_jj([
        "--config=split.identity-strategy=[\"selected\"]",
        "split",
        "-m",
        "extracted-commit",
        "-r",
    ...
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Selected changes : qpvuntsm d3f4f708 *le-signet* | extracted-commit
    Remaining changes: wqnwkozp 0de02004 first-commit
    Working copy  (@) now at: qpvuntsm d3f4f708 *le-signet* | extracted-commit
    Parent commit (@-)      : wqnwkozp 0de02004 first-commit
    [EOF]
    ");
    insta::assert_snapshot!(get_log_output(&main_dir), @"
    @  qpvuntsmwlqt false *le-signet* extracted-commit
    ○  wqnwkozpkust false first-commit
    ◆  zzzzzzzzzzzz true
    [EOF]
    ");

    // 5. Test invalid strategy produces user error
    main_dir.run_jj(["undo"]).success();
    test_env.add_config(r#"split.identity-strategy = ["invalid-strategy"]"#);
    let output = main_dir.run_jj(["split", "file2"]);
    insta::assert_snapshot!(output, @"
    ------- stderr -------
    Error: Invalid identity strategy `invalid-strategy`.
    [EOF]
    [exit status: 1]
    ");
