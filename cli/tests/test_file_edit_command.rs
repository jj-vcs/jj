...
fn test_file_edit_uses_file_name_being_edited() {
    ...
    assert!(
        std::path::Path::new(&path)
            ...
            .to_string_lossy()
            .ends_with("_foo.txt"),
        "temp file for foo.txt should end with _foo.txt, got: {path}"
    );
    ...
    assert!(
        std::path::Path::new(&path)
            ...
            .to_string_lossy()
            .ends_with("_bar.txt"),
        "temp file for dir/bar.txt should end with _bar.txt, got: {path}"
    );
}
...
fn test_file_edit_immutable() {
    ...
    insta::assert_snapshot!(output, @r"
    ------- stderr -------
    Working copy  (@) now at: rlvkpnrz be0f8891 main | main
    Parent commit (@-)      : zzzzzzzz 00000000 (empty) (no description set)
    Added 0 files, modified 1 files, removed 0 files
    [EOF]
    ");
...
fn test_file_edit_editor_fail() {
    ...
    insta::with_settings!({
        filters => [
            ...
            (r"in .*(editor-)[^.]*(file\.txt)\b", "in <redacted>$1<redacted>$2"),
        ...
        Hint: Edited file is left in <redacted>editor-<redacted>file.txt
        [EOF]
        [exit status: 1]
        ");
    });
