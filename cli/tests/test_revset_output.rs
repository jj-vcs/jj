...
fn test_default_string_pattern() {
    ...
    1 | author('test.user')
      |        ^---------^
      |
      = Default pattern syntax will be changed to `glob:` in a future release; use `exact:` or `substring:` prefix or set ui.revsets-use-glob-by-default=true to suppress this warning
    [EOF]
    "###);
