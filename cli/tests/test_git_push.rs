...
fn test_git_push_custom_revset() {
    let (test_env, workspace_root) = set_up();
    // add a custom revset wich simulates ignoring a `private()` revset.
    test_env.add_config(
        r#"
    'revsets.push(remote)' = "remote_bookmarks(remote)..@ & ~subject(glob:wip:*)"
    "#,
    );
    test_env
        .run_jj_in(
            &workspace_root,
            ["new", "bookmark2", "-m", "commit to be pushed"],
        )
        .success();
    test_env
        .run_jj_in(&workspace_root, ["new", "-m", "wip: stuff"])
        .success();
    test_env
        .run_jj_in(&workspace_root, ["bookmark", "set", "bookmark2", "-r@"])
        .success();
    test_env
        .run_jj_in(&workspace_root, ["new", "-m", "commit which should pushed"])
        .success();
    test_env
        .run_jj_in(
            &workspace_root,
            ["new", "-m", "wip: commit which should not be pushed"],
        )
        .success();
    //
    let output = test_env.run_jj_in(&workspace_root, ["log"]);
    insta::assert_snapshot!(output, @r"
