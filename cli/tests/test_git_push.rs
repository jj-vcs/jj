...
fn test_git_push_no_git_executable() {
    ...
    Changes to push to origin:
      Move sideways bookmark bookmark1 from d13ecdbda2a2 to e612d524a5c6
    Error: Could not find 'git' executable in the OS path
    "###);
}

#[test]
fn test_git_push_no_git_executable_with_path() {
    let (mut test_env, workspace_root) = set_up();
    ...
    if cfg!(windows) {
        ...
        Changes to push to origin:
          Move sideways bookmark bookmark1 from d13ecdbda2a2 to e612d524a5c6
        Error: Could not execute git executable at path 'C:\this\is\an\invalid\path'
        "###);
    } else {
        insta::assert_snapshot!(stderr, @r###"
        Changes to push to origin:
          Move sideways bookmark bookmark1 from d13ecdbda2a2 to e612d524a5c6
        Error: Could not execute git executable at path '/this/is/an/invalid/path'
        "###);
    }
}
