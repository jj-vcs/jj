...
fn test_git_push_unambigous_remote() {
    ...
    [EOF]
    ");
    // Move bookmark1 by one commit and push it to stream creating a subtle difference between it
    // and upstream.
    work_dir.run_jj(["new", "bookmark1"]).success();
    ...
    insta::assert_snapshot!(get_bookmark_output(&work_dir), @r#""#);
    // Move bookmark1 by one commit again and push it again to a remote which contains the
    // substring "stream"
    work_dir.run_jj(["new", "bookmark1"]).success();
