...
fn test_git_push_nothing(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_current_bookmark(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_parent_bookmark(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_no_matching_bookmark(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_matching_bookmark_unchanged(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_other_remote_has_bookmark(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_forward_unexpectedly_moved(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_sideways_unexpectedly_moved(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_deletion_unexpectedly_moved(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_unexpectedly_deleted(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_creation_unexpectedly_already_exists(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_locally_created_and_rewritten(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_multiple(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_changes(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_revisions(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_mixed(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_existing_long_bookmark(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_unsnapshotted_change(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_conflict(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_no_description(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_no_description_in_immutable(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_missing_author(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_missing_author_in_immutable(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_missing_committer(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_missing_committer_in_immutable(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_deleted(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_conflicting_bookmarks(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_deleted_untracked(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_tracked_vs_all(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_moved_forward_untracked(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_moved_sideways_untracked(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test_case(false; "use git2 for remote calls")]
#[test_case(true; "shell out to git for remote calls")]
fn test_git_push_to_remote_named_git(shell: bool) {
    let (test_env, workspace_root) = set_up();
    if shell {
        test_env.add_config("git.shell = true")
    }
...
#[test]
fn test_git_push_no_git_executable() {
    let (mut test_env, workspace_root) = set_up();
    test_env.add_config("git.shell = true");
    // Show the setup. `insta` has trouble if this is done inside `set_up()`
    test_env.add_config(r#"revset-aliases."immutable_heads()" = "none()""#);
    test_env.jj_cmd_ok(&workspace_root, &["edit", "bookmark1"]);
