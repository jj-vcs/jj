...
* **Branches: Yes.** You can read more about
  [how branches work in Jujutsu](../concepts/bookmarks.md)
...
* **Signed commits: Yes.**
  You can sign commits automatically [by configuration](../reference/config.md#commit-signing),
...
* Interleaving `jj` and `git` commands increases the chance of confusing branch
  ...
  ids](../concepts/glossary.md#divergent-change). These never lose data, but can be
  annoying.
...
* Colocated workspaces are less resilient to
  [concurrency](../technical-details/concurrency.md#syncing-with-rsync-nfs-dropbox-etc)
