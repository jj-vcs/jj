...
#[cfg(feature = "git")]
fn get_git_autocrlf_config(
    git_backend: &GitBackend,
) -> Option<gix::filter::plumbing::eol::AutoCrlf> {
...
pub(crate) fn get_snapshot_reader_target_eol(
    #[allow(unused)] file_path: &Path,
    #[allow(unused)] store: &Store,
) -> TargetEol {
    cfg_if::cfg_if! {
        if #[cfg(feature = "git")] {
            ...
            if let Some(git_backend) = store
                .backend_impl()
                .downcast_ref::<GitBackend>() {
                if let Some(auto_crlf) = get_git_autocrlf_config(git_backend) {
                    match auto_crlf {
...
pub(crate) async fn get_update_writer_target_eol(
    #[allow(unused)] repo_path: &RepoPath,
    #[allow(unused)] file_id: &FileId,
    #[allow(unused)] store: &Store,
) -> TargetEol {
    cfg_if::cfg_if! {
        if #[cfg(feature = "git")] {
            ...
            if let Some(git_backend) = store
                ...
                if let Some(auto_crlf) = get_git_autocrlf_config(git_backend) {
                    match auto_crlf {
                        ...
                        AutoCrlf::Enabled => {
                            match is_file_binary(repo_path, file_id, store).await {
                                Some(true) => return TargetEol::PassThrough,
