...
impl TreeBuilder {
    ...
    async fn get_base_trees(
        ...
        let mut tree_reads: FuturesUnordered<
            BoxFuture<'static, BackendResult<(RepoPathBuf, Tree)>>,
        > = FuturesUnordered::new();

        // Schedule reading the root tree
        {
            let root_tree_id = self.base_tree_id.clone();
            let store = store.clone();
            tree_reads.push(Box::pin(async move {
                let root_dir = RepoPathBuf::root();
                let tree = store.get_tree(root_dir.clone(), &root_tree_id).await?;
                Ok((root_dir, tree))
            }));
        }
                ...
                for (basename, _child) in node.children() {
                    ...
                    tree_reads.push(Box::pin(async move {
                        let sub_tree = tree
                            .sub_tree(&basename)
                            .await?
                            .unwrap_or_else(|| Tree::empty(tree.store().clone(), sub_dir.clone()));
                        Ok((sub_dir, sub_tree))
                    }));
                }
            }
