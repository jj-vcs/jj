...
impl SecureConfig {
    ...
    fn generate_initial_config(
        ...
        let path = self.generate_config(config_dir, config_id, None, &metadata)?;
        Ok((path, metadata))
    }
        ...
        if let Ok(tmp) = NamedTempFile::new_in(&self.repo_dir) {
            ...
            if !got.join(tmp.path().file_name().unwrap()).exists() {
                ...
                return Ok(LoadedSecureConfig {
                    ...
                    metadata,
                    warnings: vec![format!(
                        "Your repo appears to have been copied from {got:?} to {:?}. The \
                         corresponding repo config file has also been copied.",
                        &self.repo_dir
                    )],
                });
            };
    ...
    fn maybe_migrate_legacy_config(
        ...
        Ok(LoadedSecureConfig {
            warnings: vec![format!(
                "Your config file has been migrated from {legacy_config:?} to {config_file:?}. \
                 You can edit the new file with `jj config edit`"
            )],
