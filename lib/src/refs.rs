...
fn find_pair_to_remove(
    ...
    for (add_index1, add1) in conflict.adds().enumerate() {
        for (add_index2, add2) in conflict.adds().enumerate().skip(add_index1 + 1) {
            ...
            if let Some(remove_index) = conflict
                .removes()
                .enumerate()
                .find_map(|(i, remove)| match remove {
                    Some(id) => match index.is_ancestor(id, add_id) {
                        Ok(true) => Some(Ok(i)),
                        Ok(false) => None,
                        Err(e) => Some(Err(e)),
                    },
                    None => Some(Ok(i)), // Absent ref can be considered a root
                })
                .transpose()?
            {
