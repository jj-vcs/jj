    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        loop {
            ...
            match (before, after) {
                ...
                //
                // TODO: this will emit two diff entries, where the old diffstream would contain
                // only one. We could block on the result of the copy-tracing future, and then
                // only split the diff entry if a copy ancestor was found, but for now let's keep
                // things simple and always split the entry.
