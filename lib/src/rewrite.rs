...
/// `target_descriptions` is not empty, it will be consulted to retrieve the new
/// descriptions of the target commits, falling back to the original if the map
/// does not contain an entry for a given commit. If `children_commit_ids` is
/// not empty, the `children_commit_ids` will be rebased onto the heads of the
