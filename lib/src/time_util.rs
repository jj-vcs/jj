...
/// Extension trait for parsing timestamps.
pub trait TimestampExt {
    ...
    /// 2. If step 1 fails, attempt to parse RFC3339/ISO8601 timestamp that
    ///    include an _explicit_ offset.
    ...
    fn parse_datetime(s: &str) -> Result<Timestamp, jiff::Error> {
...
impl TimestampExt for crate::backend::Timestamp {}

fn parse_temporal_datetime(literal: &str) -> Option<Zoned> {
    let trimmed = literal.trim();
    trimmed.parse::<Zoned>().ok().or_else(|| {
        annotate_with_offset(trimmed).and_then(|annotated| annotated.parse::<Zoned>().ok())
    })
}

fn annotate_with_offset(literal: &str) -> Option<String> {
    if literal.is_empty() || literal.contains('[') {
        return None;
    }
    if literal.ends_with('Z') || literal.ends_with('z') {
        return Some(format!("{literal}[+00:00]"));
    }
    let (prefix, offset) = split_offset_candidate(literal)?;
    if !prefix.contains(':') {
        return None;
    }
    ...
    Some(format!("{literal}[{offset}]"))
}

fn split_offset_candidate(literal: &str) -> Option<(&str, &str)> {
    for (idx, ch) in literal.char_indices().rev() {
        if ch == '+' || ch == '-' {
            if idx == 0 {
                return None;
            }
            let offset = &literal[idx..];
            if offset.len() < 2 {
                return None;
            }
            let prefix = literal[..idx].trim_end();
            if prefix.contains(':') {
                return Some((prefix, offset));
            }
            break;
        }
    }
...
mod tests {
    ...
    fn test_parse_datetime_non_sense_yields_error() {
        let parse_error = Timestamp::parse_datetime("aaaaa").err().unwrap();
    ...
    fn test_parse_datetime_preserves_explicit_offset() {
        let ts = Timestamp::parse_datetime("1995-12-19T16:39:57-08:00").unwrap();
    ...
    fn test_parse_datetime_handles_z_suffix() {
        let ts = Timestamp::parse_datetime("1995-12-19T16:39:57Z").unwrap();
