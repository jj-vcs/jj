    ...
    NI: IntoIterator<Item = T>,
{
    ...
    dfs_ok(to_infallible_iter(start), id_fn, neighbors_fn).map(Result::unwrap)
}
...
/// The graph has a cycle involving the given item.
#[derive(Debug, Error)]
#[error("Cycle in graph involving {0}")]
pub struct CycleError<T>(pub T);
    ...
    NI: IntoIterator<Item = T>,
{
    ...
    topo_order_forward_ok(to_ok_iter(start), id_fn, neighbors_fn, CycleError)
}
...
{
    let neighbors_fn = move |node: &T| to_ok_iter(neighbors_fn(node));
    topo_order_reverse_ok(to_ok_iter(start), id_fn, neighbors_fn, CycleError)
}
...
pub fn topo_order_reverse_lazy<T, ID, II, NI>(
    ...
    id_fn: impl Fn(&T) -> ID,
    mut neighbors_fn: impl FnMut(&T) -> NI,
) -> impl Iterator<Item = Result<T, CycleError<T>>>
where
    T: Ord,
    ID: Hash + Eq + Clone,
    II: IntoIterator<Item = T>,
...
{
    let neighbors_fn = move |node: &T| to_ok_iter(neighbors_fn(node));
    topo_order_reverse_lazy_ok(to_ok_iter(start), id_fn, neighbors_fn, CycleError)
}
...
{
    let neighbors_fn = move |node: &T| to_ok_iter(neighbors_fn(node));
    topo_order_reverse_ord_ok(to_ok_iter(start), id_fn, neighbors_fn, CycleError)
}
...
{
    let neighbors_fn = move |node: &T| to_infallible_iter(neighbors_fn(node));
    heads_ok(to_infallible_iter(start), id_fn, neighbors_fn).unwrap()
}
...
mod tests {
    ...
    fn test_topo_order_reverse_linear() {
        ...
        let common = topo_order_reverse(vec!['C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
        let common = topo_order_reverse(vec!['C', 'B'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
        let common = topo_order_reverse(vec!['B', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
        ...
        let common = topo_order_reverse_ord(vec!['C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
        let common = topo_order_reverse_ord(vec!['C', 'B'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
        let common = topo_order_reverse_ord(vec!['B', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['C', 'B', 'A']);
    }
    ...
    fn test_topo_order_reverse_merge() {
        ...
        let common = topo_order_reverse(vec!['F'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
        let common = topo_order_reverse(vec!['F', 'E', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'D', 'E', 'C', 'B', 'A']);
        let common = topo_order_reverse(vec!['F', 'D', 'E'], id_fn, neighbors_fn).unwrap();
        ...
        let common = topo_order_reverse_ord(vec!['F'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
        let common = topo_order_reverse_ord(vec!['F', 'E', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
        let common = topo_order_reverse_ord(vec!['F', 'D', 'E'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
    }
    ...
    fn test_topo_order_reverse_nested_merges() {
        ...
        let common = topo_order_reverse(vec!['I'], id_fn, neighbors_fn).unwrap();
        ...
        let common = topo_order_reverse_ord(vec!['I'], id_fn, neighbors_fn).unwrap();
    ...
    fn test_topo_order_reverse_merge_bad_fork_order_at_root() {
        ...
        let common = topo_order_reverse(vec!['E'], id_fn, neighbors_fn).unwrap();
        ...
        let common = topo_order_reverse_ord(vec!['E'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['E', 'D', 'C', 'B', 'a']);
    }
    ...
    fn test_topo_order_reverse_merge_and_linear() {
        ...
        let common = topo_order_reverse(vec!['G'], id_fn, neighbors_fn).unwrap();
        ...
        let common = topo_order_reverse_ord(vec!['G'], id_fn, neighbors_fn).unwrap();
    ...
    fn test_topo_order_reverse_multiple_heads() {
        ...
        let common = topo_order_reverse(vec!['F', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
        ...
        let common = topo_order_reverse_ord(vec!['F', 'C'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['F', 'E', 'D', 'C', 'B', 'A']);
    }
    ...
    fn test_topo_order_reverse_multiple_roots() {
        ...
        let common = topo_order_reverse(vec!['D'], id_fn, neighbors_fn).unwrap();
        ...
        let common = topo_order_reverse_ord(vec!['D'], id_fn, neighbors_fn).unwrap();
        assert_eq!(common, vec!['D', 'C', 'B', 'A']);
    }
    ...
    fn test_topo_order_reverse_cycle_linear() {
        ...
        let result = topo_order_reverse(vec!['C'], id_fn, neighbors_fn);
        assert!(result.is_err());

        topo_order_reverse_lazy(vec!['C'], id_fn, neighbors_fn)
            .take(3)
            .collect_vec(); // sanity check
        let result: Result<Vec<_>, _> = topo_order_reverse_lazy(vec!['C'], id_fn, neighbors_fn)
            .take(4)
            .try_collect();
        assert!(result.is_err());

        let result = topo_order_reverse_ord(vec!['C'], id_fn, neighbors_fn);
        assert!(result.is_err());
    }
    ...
    fn test_topo_order_reverse_cycle_to_branchy_sub_graph() {
        ...
        let result = topo_order_reverse(vec!['D'], id_fn, neighbors_fn);
        assert!(result.is_err());

        topo_order_reverse_lazy(vec!['D'], id_fn, neighbors_fn)
            .take(4)
            .collect_vec(); // sanity check
        let result: Result<Vec<_>, _> = topo_order_reverse_lazy(vec!['D'], id_fn, neighbors_fn)
            .take(5)
            .try_collect();
        assert!(result.is_err());

        let result = topo_order_reverse_ord(vec!['D'], id_fn, neighbors_fn);
        assert!(result.is_err());
    }
    ...
    fn test_topo_order_reverse_lazy_cycle_within_branchy_sub_graph() {
        ...
        let result = topo_order_reverse_lazy(['D'], id_fn, neighbors_fn)
            .nth(1)
            .unwrap();
        assert!(result.is_err());
