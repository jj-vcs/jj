...
fn signing_key() -> Result<SigningKey, UserConfigError> {
    ...
    Ok(match fs::read(&signing_key).context(&signing_key) {
...
pub fn read_user_config<T: ConfigType>(dir: &Path) -> Result<UserConfig<T>, UserConfigError> {
    let path = dir.join(T::filename());
    let buf = match fs::read(&path).context(&path) {
        Ok(buf) => buf,
        Err(e) if e.source.kind() == io::ErrorKind::NotFound => {
            if dir.exists() {
...
pub fn write_user_config<T: ConfigType>(dir: &Path, config: &T) -> Result<(), UserConfigError> {
    ...
    let out = dir.join(T::filename());
    fs::rename(f.path(), &out).context(&out)?;
