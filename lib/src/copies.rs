...
impl From<TreeDiffEntry> for CopyHistoryTreeDiffEntry {
    // Simple conversion case where no copy tracing is needed
    fn from(diff_entry: TreeDiffEntry) -> Self {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        loop {
            ...
            let Ok(Diff { before, after }) = &next_diff_entry.values else {
                self.pending
                    .push_back(Box::pin(ready(next_diff_entry.into())));
                continue;
            };
...
async fn classify_source(
    ...
    // "normal" diff source.
    if current_history.current_path == before_path
    ...
    let after_val = tree.path_value_async(&before_path).await?;
