...
fn commit_from_git_without_root_parent(
    ...
    // valid JJ Change Id
    let change_id = extract_change_id_from_commit(&commit, id).unwrap_or_else(|e| e);
...
/// Extracts change id from commit headers. If this fails, returns `Err` with an
/// id from [`synthetic_change_id_from_git_commit_id`].
pub fn extract_change_id_from_commit(
    ...
    commit
        ...
        .ok_or_else(|| synthetic_change_id_from_git_commit_id(id))
}

/// Determenistically creates a change id based on the commit id
///
...
mod tests {
    ...
    fn change_id_parsing() {
        let zero_commit_id = CommitId::new(vec![0; HASH_LENGTH]);
        let id = |commit_object_bytes: &[u8]| {
            extract_change_id_from_commit(
                &CommitRef::from_bytes(commit_object_bytes).unwrap(),
                &zero_commit_id,
            )
        };
        ...
        insta::assert_compact_debug_snapshot!(
            id(commit_with_id),
            @r#"Ok(ChangeId("efbc06dc4721683f2a45568dbda31e99"))"#
        );
        ...
        insta::assert_compact_debug_snapshot!(
            ...
            @r#"Err(ChangeId("00000000000000000000000000000000"))"#
        );
