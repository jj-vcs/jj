...
mod tests {
    ...
    fn read_plain_git_commit(uses_tree_conflict_format: bool) {
        ...
        let git_refs = backend
            ...
            .unwrap()
            .map(|git_ref| git_ref.unwrap().peel_to_id_in_place().unwrap().detach())
            .collect_vec();
    ...
    fn read_signed_git_commit() {
        ...
        let secure_sig: BString = "here are some ASCII bytes to be used as a test \
                                   signature\n\ndefinitely not PGP\n"
            .into();
        ...
        commit
            .extra_headers
            .push(("gpgsig".into(), secure_sig.clone()));
    ...
    fn write_tree_conflicts() {
        ...
        let create_tree = |i| {
            let blob_id = git_repo
                .write_blob(format!("content {i}").as_bytes())
                .unwrap();
        ...
        assert!(git_tree
            ...
            .filter(|entry| match entry {
                Ok(e) => e.filename() != b"README",
                Err(_) => false,
            })
            .all(|entry| match entry {
                Ok(e) => e.mode().0 == 0o040000,
                Err(_) => false,
            }));
    ...
    fn commit_has_ref() {
        ...
        let git_ref_ids: Vec<_> = git_refs
            .prefixed("refs/jj/keep/")
            .unwrap()
            .map(|x| x.unwrap().peel_to_commit().unwrap().id())
            .collect();
        assert!(git_ref_ids
            .iter()
            .any(|id| id.detach() == git_id(&commit_id)));
    ...
    fn import_head_commits_duplicates() {
        ...
        assert!(git_repo
            ...
            .prefixed("refs/jj/keep/")
            .unwrap()
            .any(|git_ref| git_ref.unwrap().into_fully_peeled_id().unwrap() == git_commit_id));
    }
