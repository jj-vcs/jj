...
fn git_config_branch_section_ids_by_remote(
    ...
    config
        ...
        .flatten()
        .filter(|section| {
            let remote_values = section.values("remote").into_iter();
            let push_remote_values = section.values("pushRemote").into_iter();
            remote_values
                .chain(push_remote_values)
...
fn rename_remote_in_git_branch_config(
...
fn remove_remote_from_git_config(
    config: &mut gix::config::File,
    remote_name: &str,
) -> Result<(), GitRemoteManagementError> {
    let section_ids_to_remove = config
        ...
        .into_iter()
        .chain(git_config_branch_section_ids_by_remote(
            config,
            remote_name,
        )?);
...
pub fn remove_remote(
    ...
    let mut config = git_repo.config_snapshot().clone();
    remove_remote_from_git_config(&mut config, remote_name)?;
    save_git_config(&config).map_err(GitRemoteManagementError::GitConfigSaveError)?;
...
fn remove_remote_git_refs(
    ...
    git_repo.edit_references(
        git_repo
            ...
            .map(|result| result.map(remove_ref))
            .collect::<Result<Vec<_>, _>>()?,
    )?;
    Ok(())
}
...
pub fn rename_remote(
    ...
    remote
        ...
        .map_err(GitRemoteManagementError::from_git)?;
    rename_remote_in_git_branch_config(&mut config, old_remote_name, new_remote_name)?;
    remove_remote_from_git_config(&mut config, old_remote_name)?;
    save_git_config(&config).map_err(GitRemoteManagementError::GitConfigSaveError)?;
