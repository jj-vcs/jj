...
/// created in a temporary sibling directory, its gitlink moved into place, and
/// the recorded paths repaired.
fn add_worktree_to_populated_dir(
    ...
    let tmp_dir = tempfile::TempDir::new_in(destination.parent().unwrap_or(Path::new(".")))
        .map_err(GitCreateWorktreeError::TempDir)?;
    ...
    if let Err(err) = git_ctx.spawn_worktree_repair(destination) {
        ...
        drop(tmp_dir);
        git_ctx.spawn_worktree_prune().ok();
        return Err(err.into());
    }
    Ok(())
}
