...
fn remove_remote_git_config_sections(config: &mut gix::config::File, remote_name: &str) {
    let Some(iter) = config.sections_and_ids_by_name("remote") else {
        return;
    };
    ...
    //
    // TODO: Upstream a nicer API for this to `gix`.
    let section_ids_to_remove: Vec<_> = iter
        .filter(|(section, _)| section.header().subsection_name() == Some(BStr::new(remote_name)))
        .map(|(_, id)| id)
        .collect();
    ...
    gix::url::parse::Error: From<E>,
{
    ...
    // set-url` probably doesn’t want to preserve it anyway.
