...
impl GcSettings {
    ///
    pub fn from_user_settings(settings: &UserSettings) -> Result<Self, ConfigGetError> {
        Ok(GcSettings {
            frequency_days: settings.get_int("gc.frequency-days")?,
            operation_min_count: settings.get_int("gc.operation-min-count")?,
            operation_expiry_days: settings.get_int("gc.operation-expiry-days")?,
        })
    }
...
fn read_last_gc_time(repo_path: &Path) -> Option<SystemTime> {
    let path = last_gc_run_path(repo_path);
    let bytes = fs::read(&path).ok()?;
    let millis: i64 = str::from_utf8(&bytes).ok()?.trim().parse().ok()?;
