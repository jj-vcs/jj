...
use crate::filter::GitAttributesExt as _;
...
impl FileSnapshotter<'_> {
    ...
    async fn write_path_to_store(
        ...
        if let Some(current_tree_value) = current_tree_values.as_resolved() {
            ...
            let (file, ignore_reason) = self
                .tree_state
                .filter_strategy
                .convert_to_store(BlockingAsyncReader::new(file), repo_path, || {
                    self.git_attributes.search_filter(repo_path)
                })
                .await
                .map_err(|err| SnapshotError::Other {
    ...
    async fn write_file_to_store(
        ...
        let (file, ignore_reason) = self
            .tree_state
            .filter_strategy
            .convert_to_store(BlockingAsyncReader::new(file), path, || {
                self.git_attributes.search_filter(path)
            })
            .await
            .map_err(|err| SnapshotError::Other {
                message: "Failed to use the filter to convert the contents.".to_string(),
...
impl TreeState {
    ...
    async fn write_file(
        ...
        let contents = if apply_conversion {
            ...
            let (contents, ignore_reason) = self
                .filter_strategy
                .convert_to_working_copy(file, repo_path, || {
                    git_attributes.search_filter(repo_path)
                })
                .await
                .map_err(|err| CheckoutError::Other {
                    message: "Failed to use the filter to convert the contents.".to_string(),
    ...
    async fn write_conflict(
        ...
        let (contents, ignore_reason) = self
            .filter_strategy
            .convert_to_working_copy(contents, repo_path, || {
                git_attributes.search_filter(repo_path)
            })
            .await
            .map_err(|err| CheckoutError::Other {
