...
/// On Unix it will usually be `Exec(true|false)`, but may be ignored by a
/// config value or if we determine that the current filesystem doesn't respect
/// the executable bit.
...
impl ExecFlag {
    fn matches(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            // Always treat as equal if either is `NotDefined`.
            _ => true,
        }
    }

    /// Create a bool in an environment where we can't check `IgnoreExec`.
...
/// Whether to ignore the executable bit when comparing files. File executable
/// state is always ignored on Windows but on Unix is respected by default
/// unless we find that the filesystem doesn't support executable bits or if the
/// user configures jj to ignore them.
#[cfg(unix)]
#[derive(Debug, Clone, Copy)]
...
impl IgnoreExec {
    ...
    /// check whether executable bits are supported in the working copy's path
    /// as a default.
    ...
    fn into_flag<F: Fn() -> bool>(self, is_executable: F) -> ExecFlag {
        ...
        #[cfg(windows)]
        let (exec_flag, _, _) = (ExecFlag::NotDefined, self, is_executable); // use the variables
        exec_flag
    }

    /// Convert a flag into the executable bit to write with a closure for a
    /// default value.
    fn get_exec_bit<F: Fn() -> bool>(self, exec_flag: ExecFlag, default: F) -> bool {
        #[cfg(unix)]
        let ignore = self.0;
        #[cfg(windows)]
        let (ignore, _) = (true, self); // use the variable
        match exec_flag {
            ExecFlag::Exec(executable) if !ignore => executable,
            _ => default(),
        }
    }
}
...
impl FileState {
    ...
    /// state, ignoring materialized conflict data. Whether this file state is
    /// compatible with another file state. Ignores materialized conflict data
    /// and handles executable bit differences correctly.
...
fn file_state(metadata: &Metadata, ignore_exec: IgnoreExec) -> Option<FileState> {
    ...
    let file_type = if metadata_file_type.is_dir() {
        ...
        #[cfg(windows)]
        let exec_flag = ExecFlag::NotDefined;
...
impl FileSnapshotter<'_> {
        ...
        if file_type.is_dir() {
                ...
                && !self.start_tracking_matcher.matches(&path)
            {
                ...
                if maybe_current_file_state.is_none() && metadata.len() > self.max_new_file_size {
                    ...
                    file_state(&metadata, self.tree_state.ignore_exec)
                {
    ...
    fn visit_tracked_files(&self, file_states: FileStates<'_>) -> Result<(), SnapshotError> {
        for (tracked_path, current_file_state) in file_states {
            if matches!(current_file_state.file_type, FileType::GitSubmodule) {
                continue;
            }
            ...
            if let Some(new_file_state) = metadata
                .as_ref()
                .and_then(|f| file_state(f, self.tree_state.ignore_exec))
            {
                self.process_present_file(
    ...
    fn process_present_file(
        ...
        if maybe_current_file_state.is_none_or(|state| new_file_state.differs(state)) {
    ...
    fn get_updated_tree_value(
        ...
        if clean {
            ...
            let new_file_type = if !self.tree_state.symlink_support {
                let mut new_file_type = new_file_state.file_type;
    ...
    async fn write_path_to_store(
        ...
        if let Some(current_tree_value) = current_tree_values.as_resolved() {
            ...
            let current_exec_bit = || match current_tree_value {
            ...
            let executable = self
                .tree_state
                .ignore_exec
                .get_exec_bit(exec_flag, current_exec_bit);
...
impl TreeState {
    fn write_file(
        ...
        Ok(FileState::for_file(exec_flag, size, &metadata, None))
    }
            ...
            if after.is_absent() {
                ...
                let file_type = match after.into_resolved() {
                    ...
                    Err(values) => {
                        ...
                        // Use the most recently added filetype from the merge
...
impl LocalWorkingCopy {
    ...
    pub fn init(
        ...
        Ok(LocalWorkingCopy {
            ...
            state_path,
            checkout_state: OnceCell::new(),
            ignore_exec: tree_state.ignore_exec,
...
mod tests {
    ...
    // equal (due to `ExecFlag`), so we should not implement `PartialEq`
    // generally.
