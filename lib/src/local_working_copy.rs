...
impl ExecBit {
    ...
    ///
    /// On Windows, we set this to `false` because when we later write files, we
    /// always create them anew, so the executable bit will always be `false`
    ...
    fn new_from_repo(
        ...
        match exec_policy {
            #[cfg(windows)]
            ExecChangePolicy::Ignore => Self(false),
            #[cfg(unix)]
