...
// TODO: finish documenting `revset.pest`.
#![allow(missing_docs)]
...
/// An error which can occur during Revset parsing.
#[derive(Debug, Error)]
#[error("{pest_error}")]
...
/// The RevsetError kind, so the source the error stems from.
#[derive(Debug, Error, PartialEq, Eq)]
pub enum RevsetParseErrorKind {
    /// A plain Syntax error.
    #[error("Syntax error")]
    SyntaxError,
    /// A wrongly applied operator which isn't a PrefixOperator.
    #[error("`{op}` is not a prefix operator")]
    NotPrefixOperator {
        /// The operator.
        op: String,
        /// The nearest operator which matches for this case.
        similar_op: String,
        /// Any additional description.
        description: String,
    },
    /// A wrongly applied operator which isn't a PostfixOperator.
    ...
    /// A wrongly applied which isn't a InfixOperator.
    ...
    /// A function was not found or doesn't exist.
    #[error("Function `{name}` doesn't exist")]
    NoSuchFunction {
        /// The name of the given function.
        name: String,
        /// A list of function candidates which match for a substring of `name`.
        candidates: Vec<String>,
    },
    /// A function received wrong arguments.
    #[error("Function `{name}`: {message}")]
    InvalidFunctionArguments {
        /// The name of the function.
        name: String,
        /// An additional message which is appended to the error.
        message: String,
    },
    /// A file pattern was passed without a workspace.
    #[error("Cannot resolve file pattern without workspace")]
    FsPathWithoutWorkspace,
    /// The working-copy was requested without a workspace.
    #[error("Cannot resolve `@` without workspace")]
    WorkingCopyWithoutWorkspace,
    /// A function parameter was redefined.
    #[error("Redefinition of function parameter")]
    RedefinedFunctionParameter,
    /// An expression caused the error.
    #[error("{0}")]
    Expression(String),
    /// A alias failed to expand.
    #[error("In alias `{0}`")]
    InAliasExpansion(String),
    /// A parameter failed to expand.
    #[error("In function parameter `{0}`")]
    InParameterExpansion(String),
    /// There was recursion during the alias expansion.
    #[error("Alias `{0}` expanded recursively")]
    RecursiveAlias(String),
}

impl RevsetParseError {
    /// Create a new `RevsetParseError` with the given `kind` and `span`.
    ...
    /// Add an additional error source to the `RevsetParseError`.
...
/// Describes the kinds of Expressions the Revset parser understands.
...
pub enum ExpressionKind<'i> {
    ...
    /// `..`
    RangeAll,
    /// A urnary expression with its kind and expression.
    Unary(UnaryOp, Box<ExpressionNode<'i>>),
    /// A binary expression with its kind both primary and secondary expression.
    Binary(BinaryOp, Box<ExpressionNode<'i>>, Box<ExpressionNode<'i>>),
    /// `x | y | ..`
    UnionAll(Vec<ExpressionNode<'i>>),
    /// A function call with its specific node.
...
/// A urnary operation in the Revset language.
...
/// A binary operation in the Revset  language.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
pub enum BinaryOp {
    /// `&`
    Intersection,
...
/// A map of Revset Aliases, usually defined in the `[revset-aliases]` toml
/// table.
pub type RevsetAliasesMap = AliasesMap<RevsetAliasParser, String>;

/// A RevsetAliasesParser is responsible for parsing String expressions into
/// Revset Aliases.
...
/// Expect a `StringPattern` of `type_name` in `node`.
/// Returns the parsed string and optionally the pattern name on success.
...
/// Expect a literal in `node` with the given `type_name`.
...
/// Expect a String literal in `node` with the given `type_name`.
