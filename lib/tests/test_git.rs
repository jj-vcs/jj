...
fn test_import_refs() -> TestResult {
    ...
    git::import_head(tx.repo_mut(), WorkspaceName::DEFAULT, None).block_on()?;
...
fn test_import_refs_missing_git_commit() -> TestResult {
    ...
    let mut tx = repo.start_transaction();
    let result = git::import_head(tx.repo_mut(), WorkspaceName::DEFAULT, None).block_on();
    assert_matches!(
        result,
...
fn test_export_refs_no_detach() -> TestResult {
    ...
    git::import_head(mut_repo, WorkspaceName::DEFAULT, None).block_on()?;
...
fn test_reset_head_to_root() -> TestResult {
    ...
    git::reset_head(tx.repo_mut(), WorkspaceName::DEFAULT, &commit2, None).block_on()?;
    assert!(git_repo.head()?.is_detached(), "HEAD is detached");
    assert_eq!(
    ...
    git::reset_head(tx.repo_mut(), WorkspaceName::DEFAULT, &commit1, None).block_on()?;
    assert!(git_repo.head()?.is_unborn(), "HEAD is unborn");
...
fn test_reset_head_detached_out_of_sync() -> TestResult {
    ...
    git::reset_head(tx.repo_mut(), WorkspaceName::DEFAULT, &commit3, None).block_on()?;
    assert_eq!(
        tx.repo().git_head(WorkspaceName::DEFAULT),
        &RefTarget::normal(commit1.id().clone())
    );
    ...
    assert_matches!(
        git::reset_head(tx.repo_mut(), WorkspaceName::DEFAULT, &commit4, None).block_on(),
...
fn test_reset_head_with_index_no_conflict() -> TestResult {
    ...
    git::reset_head(mut_repo, WorkspaceName::DEFAULT, &wc_commit, None).block_on()?;
