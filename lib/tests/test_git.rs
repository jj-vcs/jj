...
fn git_fetch(
    ...
    let stats = GitFetchStats { default_branch, import_stats };
    Ok(stats)
}
...
fn test_import_refs() {
    ...
    assert_eq!(
        ...
        &RemoteRef { target: RefTarget::normal(jj_id(commit1)), state: RemoteRefState::Tracked },
    );
    ...
    assert_eq!(view.get_local_bookmark("feature3".as_ref()), &RefTarget::normal(jj_id(commit6)));
    ...
    assert_eq!(
        ...
        &RemoteRef { target: RefTarget::normal(jj_id(commit6)), state: RemoteRefState::Tracked },
    );
...
fn test_import_refs_reimport() {
    ...
    assert_eq!(
        view.get_remote_bookmark(remote_symbol("main", "origin")),
        &RemoteRef { target: commit1_target.clone(), state: RemoteRefState::Tracked },
    );
    assert_eq!(
...
fn test_export_refs_tag_changed() {
    ...
    assert_eq!(
        git_repo.find_reference("refs/tags/v1.0").unwrap().peel_to_commit().unwrap().id(),
        git_id(&new_commit)
    );
}
...
fn test_fetch_initial_commit_head_is_not_set() {
    ...
    let initial_commit_remote_ref =
        RemoteRef { target: initial_commit_target.clone(), state: RemoteRefState::Tracked };
    assert_eq!(
        *view.git_refs(),
        btreemap! {
...
fn test_fetch_success() {
    ...
    let mut tx = test_data.repo.start_transaction();
    git_fetch(tx.repo_mut(), "origin".as_ref(), vec![StringPattern::all()], &git_settings, None)
        .unwrap();
...
fn test_fetch_empty_refspecs() {
    ...
    let mut tx = test_data.repo.start_transaction();
    git_fetch(tx.repo_mut(), "origin".as_ref(), vec![], &git_settings, None).unwrap();
    assert_eq!(
        tx.repo_mut().get_remote_bookmark(remote_symbol("main", "origin")),
        RemoteRef::absent()
    );
...
fn test_push_bookmarks_success() {
    ...
    assert_eq!(
        result.unwrap(),
        GitPushStats { pushed: vec!["refs/heads/main".into()], ..Default::default() }
    );
...
#[test]
fn test_list_references_in_remote() {
    let test_data = GitRepoData::create();
    let git_settings = GitSettings::from_settings(test_data.repo.settings()).unwrap();
    let commit_main = empty_git_commit(&test_data.origin_repo, "refs/heads/main", &[]);
    let commit_feat1 =
        empty_git_commit(&test_data.origin_repo, "refs/remotes/origin/feature1", &[commit_main]);
    ...
    let result =
        git::list_references_in_remote(mut_repo, &git_settings, "origin".as_ref(), &patterns)
            .unwrap();

    let expected = [
        (jj_id(commit_main), GitRefNameBuf::from("refs/heads/main")),
        (jj_id(commit_feat1), GitRefNameBuf::from("refs/remotes/origin/feature1")),
    ];
    ...
    let expected = [(jj_id(commit_feat1), GitRefNameBuf::from("refs/remotes/origin/feature1"))];
    assert_eq!(result, expected);
}
