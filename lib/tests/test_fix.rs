...
struct TestFileFixer {
    /// If true, the fixer will use `compute_modified_line_ranges` to determine
...
#[test]
fn test_get_base_commit_map_diamond_merge() {
    let test_repo = TestRepo::init();
    let repo = &test_repo.repo;
    ...
    let expected_base_commit_map: HashMap<CommitId, IndexSet<CommitId>> = HashMap::from([
        ...
        (commit_c, IndexSet::from([commit_b.clone()])),
    ]);
...
fn test_fix_formatter_caching() {
    ...
    // The formatter should be called twice. Once for either bar and once for baz.
