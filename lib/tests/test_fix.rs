...
fn make_fix_content_error(message: &str) -> FixError {
    FixError::FixContent(anyhow!("Forcing failure: {}", message).into())
}
...
fn test_fix_one_file() {
    ...
    let mut tx = tx.commit("test").unwrap().start_transaction();
    ...
    let summary = jj_lib::fix::fix_files(
        root_commits,
        Box::new(matcher),
...
fn test_fixer_fails() {
    ...
    let result = jj_lib::fix::fix_files(
        root_commits,
        Box::new(matcher),
        include_unchanged_files,
        tx.repo_mut(),
    ...
    assert_eq!(error.to_string(), "Forcing failure: boo");
}
...
fn test_parallel_fixer_no_changes_upon_partial_failure() {
    ...
    let error = result.err().unwrap();
    assert_eq!(error.to_string(), "Forcing failure: boo7");
}
