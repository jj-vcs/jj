...
fn test_annotate_linear() {
    ...
    let tree1 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content1);
    });
    let tree2 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content2);
    });
    let tree3 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content3);
    });
...
fn test_annotate_merge_simple() {
    ...
    let tree4 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content4);
    });
    let commit1 = create_commit("commit1", &[root_commit_id], tree1.id());
    let commit2 = create_commit("commit2", &[commit1.id()], tree2.id());
...
fn test_annotate_merge_split_interleaved() {
    ...
    let tree5 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content5);
    });
    let tree6 = create_tree(repo, |builder| {
        builder.entry(file_path).text_file(content6);
    });
    let commit1 = create_commit("commit1", &[root_commit_id], tree1.id());
