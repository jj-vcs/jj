...
/// the new tree id.
#[track_caller]
fn create_tree_executable(store: &Arc<Store>, repo_path: &RepoPath, executable: bool) -> TreeId {
    let mut tree_builder = TreeBuilder::new(store.clone(), store.empty_tree_id().clone());
    let empty_content_id = store
        .write_file(repo_path, &mut (&[] as &[u8]))
        .block_on()
        .unwrap();
    tree_builder.set(
        repo_path.to_owned(),
    ...
    tree_builder.write_tree().unwrap()
}
