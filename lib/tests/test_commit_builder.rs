...
use jj_lib::time_util::TimestampExt as _;
...
fn test_rewrite_resets_author_timestamp(backend: TestRepoBackend) {
    ...
    let initial_timestamp = Timestamp::parse_datetime(initial_timestamp).unwrap();
    ...
    let new_timestamp_1 = Timestamp::parse_datetime(new_timestamp_1).unwrap();
    ...
    let new_timestamp_2 = Timestamp::parse_datetime(new_timestamp_2).unwrap();
