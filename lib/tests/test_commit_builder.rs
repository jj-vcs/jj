...
fn test_initial(backend: TestRepoBackend) {
    ...
    assert_matches!(
        repo.operation().predecessors_by_commit(commit.id()),
        Some([])
    );
}
...
fn test_rewrite(backend: TestRepoBackend) {
    ...
    assert_matches!(
        repo.operation().predecessors_by_commit(rewritten_commit.id()),
        Some([id]) if id == initial_commit.id()
    );
    assert_matches!(
        repo.operation().predecessors_by_commit(initial_commit.id()),
        None
    );
}
