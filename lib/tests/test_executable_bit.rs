...
/// Set the executable bit of a file in the tree and return the new tree id.
#[track_caller]
fn set_tree_executable(tree: &Tree, repo_path: &RepoPath, executable: bool) -> TreeId {
    let mut tree_val = tree.path_value(repo_path).unwrap().unwrap();
    if let TreeValue::File { executable: e, .. } = &mut tree_val {
        *e = executable;
    } else {
        panic!();
    }
    ...
    builder.set(repo_path.to_owned(), tree_val);
...
fn prepare_exec_commits(ws: &TestWorkspace, repo_path: &RepoPath) -> (Commit, Commit) {
    ...
    let tree_exec = set_tree_executable(&tree, repo_path, true);
    assert_ne!(tree_exec, tree_no_exec);

    let commit_with_id =
        |id| testutils::commit_with_tree(ws.repo.store(), MergedTreeId::resolved(id));
...
fn test_exec_bit_snapshot() {
    ...
    let mut snapshot_assert_exec_bit = |expected| {
        let tree = ws.snapshot().unwrap().take().into_resolved().unwrap();
        let tree_val = tree.path_value(repo_path).unwrap().unwrap();
    ...
    file.write_all(b"first change").unwrap();
    snapshot_assert_exec_bit(false);

    set_file_executable(path, true);
    snapshot_assert_exec_bit(true);

    file.write_all(b"second change").unwrap();
    snapshot_assert_exec_bit(true);
