...
fn flip_tree_exec_bit(tree: &Tree, repo_path: &RepoPathComponent) -> TreeId {
    let store = tree.store();
    let mut builder = store.tree_builder(store.empty_tree_id().clone());
    let tree_val = tree.value(repo_path).unwrap().clone();
    let TreeValue::File { id, mut executable } = tree_val else {
        panic!()
    };
    executable = !executable;
    builder.set(
        RepoPath::root().join(repo_path),
...
/// Checkout a commit with the given checkout options.
fn checkout_with_opts(ws: &TestWorkspace, commit: &Commit, options: &CheckoutOptions) {
    let mut locked_wc = ws.workspace.working_copy().start_mutation().unwrap();
    locked_wc.check_out(commit, options).unwrap();
...
fn test_exec_bit_snapshot() {
    ...
    let val = tree.entry(NAME.into()).unwrap().value();
...
fn test_exec_bit_checkout() {
    ...
    let path = &ws.workspace.workspace_root().to_owned().join(NAME);
    let repo_path = RepoPath::from_internal_string(NAME);
    ...
    let tree_exec = flip_tree_exec_bit(&tree, NAME.into());
