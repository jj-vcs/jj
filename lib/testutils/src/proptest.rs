...
impl Arbitrary for DirEntry {
    type Parameters = ();
    type Strategy = BoxedStrategy<DirEntry>;

    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        // TODO: Currently only creates `File`s; extend test to other variants.
        (arb_file_contents(), any::<bool>())
            .prop_map(|(contents, executable)| DirEntry::File {
                contents,
                executable,
            })
            .boxed()
    ...
    pub fn create_tree(&self, repo: &Arc<ReadonlyRepo>) -> MergedTree {
        create_tree(repo, |builder| {
            for (path, dir_entry) in &self.entries {
                let entry_builder = builder.entry(path);
                match dir_entry.clone() {
                    DirEntry::File {
                        contents,
                        executable,
                    } => {
                        entry_builder.binary_file(contents).executable(executable);
                    }
                    DirEntry::Symlink { target } => entry_builder.symlink(&target),
                    DirEntry::GitSubmodule { commit } => entry_builder.submodule(commit),
                }
            }
        })
    }
