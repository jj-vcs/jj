...
* A user-friendly CLI
* A "[revset](../reference/revsets.md)" language for selecting revisions
...
* Flexible customization of output using [templates](../reference/templates.md)
...
* **Working copy:** When using Sapling (like most VCSs), the
  ...
  is [automatically snapshotted by every command](../concepts/working-copy.md). New files
...
* **Conflicts:** Like most VCSs, Sapling requires the user to
  ...
  you [commit conflicts](../concepts/conflicts.md). Note that it's a representation of the
...
* **Undo:** jj's undo is powered by [the operation log](../concepts/operation-log.md), which
