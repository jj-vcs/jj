...
The words "revisions" and "revisions" are used interchangeably in this document.
...
The `@` expression refers to the working copy revision in the current workspace.
Use `<workspace name>@` to refer to the working-copy revision in another
...
* `working_copies()`: The working copy revisions across all the workspaces.
...
revision if `xyz` has more than one child. The `all:` modifier confirms that the
user expected `xyz` to have more than one child.

A more useful example: if `w` is a merge revision, `jj rebase -s w -d all:w- -d
xyz` will add `xyz` to the list of `w`'s parents.

## Examples

Show the parent(s) of the working-copy revision (like `git log -1 HEAD`):
...
Show some important revisions (like `git --simplify-by-decoration`):
