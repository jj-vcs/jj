...
Operators are listed in order of precedence from highest to lowest, e.g. `x | y
& z` is interpreted as `x | (y & z)` since `&` has higher precedence than
`|`. Operators of the same precedence are parsed from left to right, e.g. `x ~ y
& z` is interpreted as `(x ~ y) & z` rather than `x ~ (y & z)`.

Parentheses can be used to control evaluation order, e.g. `(x & y) | z` or `x &
(y | z)`.

<!-- https://github.com/jj-vcs/jj/blob/f06cd9b61560eebce8dbc699a0c7337c18a25873/lib/src/revset_parser.rs#L550-L572 -->
