...
* The `allow_macros` parameter is set to `true` only for the root `gitattributes` file, i.e., `path` has a parent, because Git only allows macro definition [in limited files](https://git-scm.com/docs/gitattributes#_defining_macro_attributes):
...
Before we have an answer to all these related features, `jj` always use the text based merge algorithm on the contents from the store without any conversion.
