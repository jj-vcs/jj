...
*   If we are not in a TUI, we warn them that we are ignoring the repo config (we
    return `DISABLED`), and that they can run `jj` in a terminal to configure this.
...
Notify is not a part of the MVP, and will be added later, in order to keep the
MVP simple.
...
*   if `mtime(repo_config_file) < mtime(managed config)`
    ...
    *   We print a warning, potentially diffing `last_managed_config` with the
        actual managed config on disk.
...
The notify option has a rather painful UX when it comes to keeping it in sync
...
--disable/notify/trust`. This is not a part of the MVP.
...
We considered an approval based mechanism where when you saw a config you hadn't
seen before, you would appprove or reject it. It turned out to have a lot of
edge cases though, and was extremely complex (for both the user and the
implementer). For example:
* What happens if you approve some config, then sync to an old commit?
* What happens if you have two workspaces synced to different commits?
* What happens if you want to approve some of the changes but not others?
* How do you manage approval for short-lived workspaces such as from `jj run` or
  agentic coding.
* What happens if you happen to have already seen that config already?
* Are approvals per-repo or per-workspace?
* If you don't have approval for a repo/workspace config, what do you fall back
  to?
  * Approved config for the repo?
  * Last approved config?
  * No Config

It was clear that an approval based mechanism would solve the problem, but that
the user would not be able to understand what config was being used and why.
While the current approach is a toggle on/off, the approval based mechanism was
