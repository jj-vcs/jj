...
*   If we are not in a TUI, we warn them that we are ignoring the repo config
    (we return `IGNORE`), and that they can run
    `jj config managed --ignore/review/trust` in a terminal to configure this.
