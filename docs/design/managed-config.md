...
* A repo that defines `trunk()` could run `jj rebase -b <revset> -d trunk()`
  * This would result in it going to the wrong trunk
* A repo that defines `immutable_heads() = ...` could run something like
  `jj rebase -b mutable() -d trunk()`
  * This would rebase a bunch of immutable commits
* A repo that defines `pre-upload` hooks (which don't yet exist, but are being
  planned) runs `jj gerrit upload` and bypasses them entirely. 

The above examples are all destructive, but thanks to the magic of `jj undo`,
everything except the upload is recoverable. What makes them a problem,
however, is the fact that a user would see the message "please approve /
reject the config" and not suspect that the command has failed.
...
with managed config (most users with managed config will be on a personal repo
or the chromium repo for now, I suspect).
