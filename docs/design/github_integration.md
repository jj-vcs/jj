...
Create a GitHub pull request from a revision. The command will automatically determine the appropriate base branch by finding the closest ancestor bookmark that has been pushed to a remote. If the target revision doesn't have a bookmark, one will be created using the same naming scheme as `jj git push -c`.

The command will show a preview of all actions (branch push and PR creation) and prompt for confirmation before proceeding.

**Options:**

* `-r, --revisions <REVISIONS>` — The revision to create a PR for

  Default: `@` (current revision)

* `--base <BASE>` — Override the automatically detected base branch

  Can be specified as `branch` (same repository) or `remote:branch` (cross-repository PR). By default, the base is determined by finding the closest ancestor bookmark that exists on a remote. If no ancestor bookmark is found, falls back to the default branch (main/master).

* `--remote <REMOTE>` — The remote to push the branch to

  This only controls where the branch will be pushed. The PR repository is determined by the base branch (if specified as `remote:branch`) or defaults to the same repository as the push remote. Defaults to the `git.push` setting, or "origin" if multiple remotes exist and no push default is configured.

* `--title <TITLE>` — Override the PR title

  Default: First line of the commit message

* `--body <BODY>` — Override the PR description

  Default: Uses `.github/pull_request_template.md` if present, otherwise the full commit description
...
# Cross-repository PR: push to origin, create PR in upstream
jj github pr create --remote origin --base upstream:main

# Push to different remote but same repo PR
jj github pr create --remote upstream

# Preview without taking action
jj github pr create --dry-run
```
