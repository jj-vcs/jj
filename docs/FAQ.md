...
Let's say we are editing a change for featureA, and we forgot to run `jj new` or
`jj commit` before doing some work for featureB, which belongs in a new change.
We therefore want to split the current change into two changes. One for featureA
and one for featureB.

The following diagram illustrates the change we want to make, from the left tree
to the right tree.

```
                            @ vvu "featureB"
                            │
@ xyz "featureA"     =>     ○ xyz "featureA"
│                           │
○ yyv                       ○ yyv
```

CHECK RENDER OF LINK IN INFO BOX

!!! info "Alternatives to using commit IDs"
    The following examples use commit IDs, but *most* steps can also be done
    using [change offsets][glossary_change_offset] (e.g. `lvnv/1`) or bookmark
    names (`feat-a`, `feat-a@origin`).

#### Easy: Move modifications to a change on top

This is one of the easiest methods to move changes to the correct commit. It
**does not** preserve the commit ID of the original change. Note that you could
also use `jj split` or `jj split -i` if the changes are on distinct lines or
files.

The following diagram illustrates in more detail what we will do:
```
@ xyz "featureA" (c0ab)                         @ vvu "featureB" (b626)
│                                               │
│ ○ xyz/2 (hidden) "featureA" (aa12)     =>     ○ xyz "featureA" (59dd)
├─╯                                             │
○ yyv                                           ○ yyv
```
...
```bash
# Note down the revision in which featureA was completed in the evolution log.
# E.g. aa12fe34
$ jj evolog --patch
# Create a new change for featureB on top of the current change, and move to it.
$ jj new -m "featureB"
# Set the contents of the previous change to the contents of featureA, while
# keeping the contents of the working copy intact (featureB contents).
$ jj restore --from aa12fe34 --into @- --restore-descendants
```

LINK TO RESTORE DESCENDANTS
Without `--restore-descendants`, the newer change would end up empty, as the
diff between the older and the newer change after `jj new` is empty.

If a remote bookmark points to the old revision, you can use the bookmark's name
instead of checking the evolution log (e.g. `--from feat-a@origin`).

#### Advanced: Branch from original revision, to preserve commit/revision ID

This variant starts from the featureA revision and puts the featureB changes on
top of it, preserving the commit ID of the featureA change. This is useful if
the featureA revision has been pushed to a remote tracked branch and you don't
want the commit ID to change.

The following diagram illustrates in more detail what we will do:
```
@ xyz "featureA" (c0ab)                         @ vvu "featureB" (b626)
│                                               │
│ ○ xyz/2 (hidden) "featureA" (aa12)     =>     ○ xyz "featureA" (aa12) <-- preserved!
├─╯                                             │
○ yyv                                           ○ yyv
```

```bash
# Note down the revision of the current, bad, change (c0ab7890) and featureA
# (aa12fe34) from the evolution log.
$ jj evolog --patch
# Create a new change for featureB on top of the featureA revision. The change
# will be diverged.
$ jj new aa12fe34 -m "featureB"
# Restore the contents from the bad revision into the working copy.
$ jj restore --from c0ab7890

# Move bookmarks to featureA.
$ jj bookmark move --from c0ab7890 --to aa12fe34 --allow-backwards
# Resolve the divergence by dropping the bad revision.
$ jj abandon c0ab7890
```
