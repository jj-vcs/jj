...
There are a few way os specifying the target remote for your projects:

- Please run `jj config set --user gerrit.default_for <branch name>` to set your
  default branch across all repos
- Please run `jj config set --repo gerrit.default_for <branch name>` to set your
...
- If you have more than one origin, or the origin isn't called gerrit, run
  `jj config set --repo gerrit.default_remote <gerrit remote name>` to setup a
  default remote.
