...
The exact paths are platform-specific. The per-repo and per-workspace
`config.toml` files are in generated `jj/repos/<ID>/` and `jj/workspaces/<ID>/`
subdirectories, respectively, under the platform config directory used by `jj`
(`$XDG_CONFIG_HOME` or `$HOME/.config` on Linux and macOS,
`{FOLDERID_RoamingAppData}` on Windows).
Unlike user config files, these paths are not affected by the `JJ_CONFIG`
environment variable. Each file is created when its config is first edited, such
as with `jj config edit --repo` or `jj config edit --workspace`.
