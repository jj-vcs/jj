...
it for files as you operate on a repository. If you update your working copy to
a commit where a file is recorded as executable or not, `jj` will adjust the
permission of that file. If you change a file's executable bit through the
filesystem, `jj` will record that change in the working-copy commit on the next
snapshot.
...
Windows users accessing files from WSL, or anyone experimenting with filesystem
configurations.
