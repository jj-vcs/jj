...
Similar to the [`core.autocrlf`][git-autocrlf] git config.

Similar to git, the line endings conversion won't be applied to files detected
as binary files via a heuristics[^1] regardless of the settings.
...
[^1]: To detect if a file is binary, Jujutsu currently checks if there is 0 byte
