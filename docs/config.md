...
the values have the following properties:
 ...
 - `skip-tool-if-zero-line-ranges`: Whether to skip the formatting step for this tool on
...
Additionally, you can use `skip-tool-if-zero-line-ranges` to control the
behavior when a diff results in zero line ranges (which can happen when lines
are only deleted or with `--include-unchanged-files`). If `true` (the default),
`jj` will skip running the tool on that file. If `false`, `jj` will run the tool
