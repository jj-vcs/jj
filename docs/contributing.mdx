...
The documentation website is built with [Astro](https://astro.build/) and
[Starlight](https://starlight.astro.build/). You need Node.js 22 or newer.

### Build the docs

The canonical documentation files live in `docs/`. To preview them locally,
run:
...
[http://localhost:4321](http://localhost:4321), in your browser.

As you edit the `.md` and `.mdx` files in `docs/`, the website should be
rebuilt and reloaded in your browser automatically.

:::note[If the docs are not updating]
Check the terminal from which you ran `npm run dev` for any build
...
[https://docs.jj-vcs.dev/latest](https://docs.jj-vcs.dev/latest), which in
turn redirects to the docs for the last stable version.

The different versions of documentation are managed by
`.github/scripts/docs-build-deploy`. The script builds the Starlight site,
copies it to the requested version directory on the `gh-pages` bookmark, and
...
4. Install Node.js as explained in [Previewing the HTML
documentation](#previewing-the-html-documentation), and run the same `sh`
script that is used in GitHub CI:
...
6. (Optional) The previous steps only rebuild
`https://jjfan.github.io/jj/prerelease/`. If you'd like to test out version
switching back and forth, you can also rebuild the docs for the latest release
as follows.
...
7. (Optional) When you are done, you may want to reset the `gh-pages` bookmark
...
The first argument is the version directory to deploy, such as `prerelease` or
`v1.33.1`. Positional arguments after the version are aliases to copy to, such
as `latest`. The `--push` flag pushes the updated `gh-pages` bookmark.

You can also edit the `gh-pages` bookmark directly, but take care to avoid files
that will be overwritten by future invocations of `docs-build-deploy`. Then, you
can submit a PR based on the `gh-pages` bookmark of
