...
`fdatasync`. A likely curlprit is the presence of [global kernel locks in
APFS](https://gregoryszorc.com/blog/2018/10/29/global-kernel-locks-in-apfs/).
There are some benchmarks in [this
...
To build the docs, run while inside the `web/` directory:
...
file system, run while inside the `web/` directory:
...
1. For your fork, the website will be served at `https://jjfan.github.io/jj/`.
   ...
   - In the script `.github/scripts/docs-build-deploy`, change the value of
     `--base` to `"/jj/$site/"`.
   - In `web/astro.config.mjs`, change the value of `site` from
     `https://docs.jj-vcs.dev` to `https://jjfan.github.io`.
   - In `web/src/components/VersionSwitcher.astro`, change the value of
